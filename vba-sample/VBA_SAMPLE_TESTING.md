# vba-sample パッケージテストワークスペース

## 目的

`vba-sample/` を Extension Development Host の独立ワークスペースとして使い、
...
│   └── VariantDemo.bas
└── .vscode/
    ├── launch.json        ← "Run Extension (vba-sample)" 設定済み
    └── tasks.json         ← "setup: vba-sample" タスク設定済み
```

## F5 で起動する

1. VS Code のデバッグパネルで **"Run Extension (vba-sample)"** を選択
2. F5 を押す

F5 を押すと `preLaunchTask: "setup: vba-sample"` が自動で以下を実行する。
...
4. npm install --prefix vba-sample  → node_modules/vba-runner/ に展開
5. npm run build:extension → dist/extension.cjs を生成
```

完了後、Extension Development Host が `vba-sample/` のみを開いた状態で起動する。

## この構成で何が保証されるか

| 観点 | 状態 |
|------|------|
| EH ウィンドウが参照するファイル | `vba-sample/` 内のファイルのみ |
