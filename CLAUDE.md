...
| `build/extractor/` | CLI ツール `vba-extractor`（Office ファイルから VBA ソースを入出力） | `tools/extractor/` |
...
**テスト実行**（tsx でそのまま実行）:
```bash
npx tsx tests/spec/eval-call-scope.test.ts   # 個別テスト（コミット前の確認に使う）
...
**全テスト一括実行**（コミット後に必ず使う）:
```bash
./scripts/bg-test.sh   # コミット済み HEAD を worktree で並列実行（run_in_background: true で呼ぶ）
```
...
npm run build:extractor    # build/extractor/dist/
...
| `tests/vba/` | 実 VBA 環境で動作する `.bas` 形式のテスト（`run_vba_tests.sh` で実行）|
...
本プロジェクトの主目的は**リファクタリングとユニットテストの実行**のため、ホスト環境に副作用を与える操作はスタブ・フェイク実装にする。

| 操作 | 実装方針 |
|---|---|
| 外部コマンド（`Shell`） | 実行せずログ出力のみ。成功タスク ID（`1`）を返す |
| 外部オブジェクト（`CreateObject`, `GetObject`） | 実際の COM 生成・ネットワーク通信は行わず、スタブ/フェイクを返す |
| レジストリ（`GetSetting`, `SaveSetting`） | Windows レジストリにアクセスせず、メモリ上の仮想レジストリを使う |
| アプリ状態（`AppActivate`, `SendKeys`） | no-op またはログ出力のみ |
| ユーザー対話（`MsgBox`, `InputBox`） | 実行をブロックせず、ログ出力とデフォルト値の返却のみ |
