...
- `sample/src/refactoring/` - TaskScheduler リファクタリング実験用コード。
  ...
  - `refactoring_quality/`: v2〜v5 の各手法によるリファクタリング実験ファイル群。
...
`Stop` ステートメントは、現在の実装では `console.log` に `STOP Statement encountered` と出力するのみで、プログラムの実行自体はそのまま継続されます（デバッガーのブレークポイントのような動作は行いません）。
...
`VBARunner` に対して、ディレクトリを指定して複数のモジュールをまとめてロードできます。その際、以下の拡張子のファイルを対象とします。
...
## UI画面の起動
ブラウザ上でVBAエディター環境を立ち上げ、`Debug.Print` クラスの動作などを確認します。
...
特定のテストファイル（例: サンプルのユニットテストや特定の仕様テスト）は `tsx` で直接実行します。
...
npx tsx tests/spec/mid-variants.test.ts
```
...
- **用途**: 事前に定義したロジックを、異なるパラメーターで繰り返しテストする場合に適しています。
...
```bash
# tsx で直接実行
npx tsx test-libs/vba-test-generator.ts tests/spec/vba/CurrencyOperationsTest.bas tests/spec/vba/CurrencyOperationsTest_runner.bas

# モジュール修飾なし
npx tsx test-libs/vba-test-generator.ts tests/spec/vba/CurrencyOperationsTest.bas tests/spec/vba/CurrencyOperationsTest_runner.bas --no-module-qualifier
```
...
```bash
# モジュール修飾あり（デフォルト）
npx tsx test-libs/vba-test-generator.ts tests/spec/vba/CurrencyOperationsTest.bas

# モジュール修飾なし
npx tsx test-libs/vba-test-generator.ts tests/spec/vba/CurrencyOperationsTest.bas --no-module-qualifier
```

#### 3. ディレクトリ内のすべてのテストファイルを処理

```bash
npx tsx test-libs/vba-test-generator.ts --dir tests/spec/vba
```

`tests/spec/vba/` ディレクトリ内のすべての `.bas` ファイルに対してランナーを生成し、各ファイルの隣に `<filename>_runner.bas` として保存します。サブディレクトリは個別に実行してください：

```bash
npx tsx test-libs/vba-test-generator.ts --dir tests/spec/vba/Circular
```
...
2. **テストファイルと生成されたテストランナーをインポート**
   ...
   - VBA エディターでモジュールを新規作成
   - テストランナーコードをペースト
3. **`RunAllTests` Sub を実行** （F5 キーまたは「実行」ボタン）
4. **結果を確認**
   - メッセージボックスに結果が表示されます
   - `Debug.Print` 出力は VBA エディターのイミディエイトウィンドウに表示されます
