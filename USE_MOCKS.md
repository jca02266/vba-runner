...
```typescript
import type { VbaType, VbaDefaultProperty, VbaIterable, VbaExternalObject } from '../src/engine/mock/MockExcel';
```
...
| `VbaExternalObject` | `__className__: string` | `CreateObject(progId)` の自動別名登録 | `MockWordApplication` |

### VbaExternalObject — CreateObject 自動別名登録
