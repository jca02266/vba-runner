...
Core.bas はテストラッパー（`TestFindLockedTaskFinish` / `TestScheduleUnlockedTask`）を持ち、TypeScript の VBA Runner から個々の関数を直接テストできる。全関数 Public なのでユニットテストが容易。

v5.bas はほぼ全関数が `Private` のため、ユニットテストのエントリーポイントが `AutoScheduleTasks` しかない。個別関数のテストには修飾子の変更が必要。
...
|---|---|---|
| テスト可能性 | ✅ 優（Public + テストラッパー） | ⚠️ Private 多数で困難 |
...
| テスト可能性 | ✅ Public + テストラッパーで容易 | ❌ Private のためラッパーなしには困難 |
