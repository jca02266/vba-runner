...
| # | ドメイン | 主にテストした機能 | 日付 |
|---|---|---|---|
| 87 | 実行互換性 #87: `Erase` 後の型付き配列再確保 | **Bug 87-A 修正済み**: `Erase widgets` 後の `ReDim widgets(0)` が `Variant():False` になった。未割当状態とは別に配列の宣言型を環境へ保持し、クラス配列の `Widget():True` と Integer 配列の型強制を維持する回帰テストを追加した。 | 2026-07-24 |
