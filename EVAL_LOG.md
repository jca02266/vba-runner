...
| # | ドメイン | 主にテストした機能 | 日付 |
|---|---|---|---|
| 88 | 実行互換性 #88: 型付き配列 Function の戻り値 | **Bug 88-A/B 修正済み**: `Function Make() As Integer()` 内の `Make(0) = ...` と UDT の `Make(0).X = ...` を手続き呼び出しと誤認して Error 450 にし、`ReDim` 後も Variant 配列になった。代入左辺の添字を識別し、戻り値宣言から要素型を復元する回帰テストを追加した。 | 2026-07-25 |
