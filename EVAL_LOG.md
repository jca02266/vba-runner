...
| # | ドメイン | 主にテストした機能 | 日付 |
|---|---|---|---|
| 91 | 実行互換性 #91: 型付き配列バイナリ I/O | **Bug 91-A/B/C 修正済み**: Byte 配列と Integer 配列を `Put` / `Get` に渡すと Error 13 になり、多次元 Byte 配列も未対応だった。宣言添字順の連続バイト列として往復する一次元・二次元の回帰テストを追加した。 | 2026-07-25 |
