...
| MUT-ENGINE | #129、#163、#168 | `format.ts` 丸め、`parser.ts` 比較、`evaluator.ts` If/On Errorの代表変異、UDT Binary Put/Get の配列・EOF境界変異、標準ファイルI/OのCP932テキスト・Binary混在境界。全体テストで検出 | `On Error Resume Next` の単独テスト強化、ファイルI/OのUnicode/Append/Input追加境界変異、別テスト単位での検出確認 | 継続 |
