...
|---|---|---|---|---|
| FZ-BUILTIN | 導入時（359件検出→修正、0件到達） | 組み込み関数の敵対値スモーク | `builtins.ts` / `coerce.ts` 変更後の再実行、未確認の引数位置・戻り値境界 | 継続 |
| FZ-GRAMMAR | #128、#130、#131 | If/For/Select、On Error、ReDim、Property、クラス、複数モジュール、演算子境界、宣言型サフィックス付き引数、クラス配列要素のByRef書き戻し（カバレッジ基準 2026-07-18） | 別シードでのProperty Set/Let引数のByRef境界、型付き配列の多次元・Preserve組み合わせ、宣言サフィックスと比較・連結の追加組み合わせ | 継続 |
