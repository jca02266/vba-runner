...
| 125 | 計測基盤 #125: カバレッジ集計の未完了 | テスト実行フェーズは TS 331/331、VBA 14/14・111/111 で成功した。一方、`coverage.sh` の後段（V8 JSON 1022プロセスを `c8 report` で個別変換しユニオン集計する処理）が長時間化したため中断し、今回の行カバレッジ率と未到達行一覧は未確定。これはテスト結果とは別の未完了項目として記録し、次回は集計処理の高速化または分割実行を確認する。 | 2026-07-26 |
