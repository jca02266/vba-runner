...
### C-1: 未使用変数の診断表示 ❌ 未着手（優先度：高）
未使用変数を波線で常時表示する。
- **現状**: 「Remove Unused Variables」リファクタリングコマンドはあるが、診断として常時表示されていない。保存のたびに確認が必要でコーディング中に気づけない。
- **価値**: リファクタリング機能と対になる自然な完成形。
...
| 優先 | ID | 状態 | 理由 |
|------|----|----|------|
| 高 | C-1 未使用変数の診断表示 | ❌ | リファクタリング機能と対になる自然な完成形 |
