...
「次回 `evaluate-vba-runner` の未実施領域」が空でも評価を終了しない。まず
`EVAL_LOG.md` の「継続評価キャンペーン」を確認し、`状態=継続` のファザーまたは
ミューテーションから、前回と異なる次回対象を1つ選ぶ。ファザーは生成シード・構文
組み合わせ・入力境界を、ミューテーションは対象ファイル・変異演算子・テスト単位を
前回から変更すること。同じ対象を再実行する場合は、回帰確認である理由を明記する。

実施後は、評価済みドメイン表だけでなくキャンペーン表の「実施履歴」「確認範囲」
「次回の未実施対象」を更新する。したがって、1回のファザー生成やミューテーション
実行を手法全体の完了とは記録しない。未実施対象がなくなった場合も、次のシードまたは
別の高リスクファイル・変異種別を候補として追加し、継続状態を維持する。
...
行う。選定時には、使用したカバレッジの日付と未通過箇所を `EVAL_LOG.md` の実施履歴に
残す。カバレッジは仕様適合やバグの不存在を証明しないため、実機照合結果と既知の制限も
併せて確認する。
