...
直近の評価では、Parser・Lexerの境界条件で、必須終端や必須要素をtoken返却前に検証していない経路から複数のバグを検出した。配列・Property Getの再評価では、同じ直接経路の再現性を確認したが、新規バグは確認されなかった。

今後は、単発の代表値だけでなく、仕様上の必須条件を構成要素ごとの境界マトリクスとして評価する。バグを検出した場合は、同じ分岐・責務・変換を通る隣接経路を横展開し、直接原因だけでなく、必須条件や不変条件を共通契約として表現できているかを真因分析する。確認済み、除外済み、未確定の経路を分け、未確定の実機依存部分はExcel照合キューへ送る。

評価を収束させる際は、次の順で優先する。

1. 確定した真因の対処と、未解決の真因分析
2. 確定バグの横展開漏れ
3. 実機・仕様待ちの境界
4. 通常のcoverage-guided探索

同じ原因の隣接候補は別EVとして登録し、元のEVへ混在させない。各修正には元事象と横展開経路を覆う回帰テストを追加し、監査・状態履歴・EVAL_LOG・HTMLレポートを同じループで更新する。ファザーやミューテーションは一回の実行で完了扱いにせず、未通過境界と既知原因への到達状況を次の候補選定へ反映する。
...
- **`docs/implementation/VBA_SPEC_LIST.md` / `docs/todo/TODO.md` / git log を事前に読ませない**こと（既知の課題リストに
