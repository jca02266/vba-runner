...
| 利用場面 | ユーザーがやること | AI がやること（MCP 経由） |
|---|---|---|
| **コーディング依頼** | 「この処理を Sub に切り出して」 | コードを書いてファイルに反映 |
| **テスト実行依頼** | 「テストを実行して確認して」 | `vba_run_tests` → 結果を報告 |
| **ソース評価** | 「このファイルの品質を評価して」 | `vba_analyze` + `vba_get_diagnostics` → レポートを提示 |
| **リファクタリング依頼** | 「この関数を整理して」 | outline 確認 → 変更 → テスト → 整形 → 報告 |
| **編集補助** | VBA を書く | Hover / Completion / Diagnostics でリアルタイム支援 |

AI は MCP ツールを使って **自律的にサイクルを回す**。ユーザーは結果だけ確認すればよい。
...
ユーザー: 「このSubを分割してテストも追加して」
    ↓
AI: vba_get_outline()       全体構造を把握
AI: vba_run_tests()         変更前のベースラインを確認
AI: （コードを編集）
AI: vba_format(fix=true)    整形を自動適用
AI: vba_run_tests()         テストが通ることを確認
AI: vba_analyze()           品質指標が改善したことを確認
    ↓
ユーザー: 「完了。テスト全通過・CC 改善を確認しました」を受け取る
```

MCP サーバーが整備されれば、**リポジトリを clone せず拡張機能をインストールするだけ**でこのサイクルが動く。
...
- [ ] **Code Actions（右クリック → Refactor メニュー）**
  ...
  | **✅ 関数の抽出**（Extract Sub/Function） | ✅ `LSPServer.getCodeActions()` 実装済み。選択行範囲を `analyzeDefUse` で解析し `ByVal`/`ByRef` パラメーターと `Dim` ローカル宣言を含む雛形 Sub を生成。`vba-runner.doExtractFunction` コマンドに `result + procSignature + callStatement` を渡す | ★★★ |
...
- [x] **VBA 固有の落とし穴に対する Diagnostics 警告** — `src/engine/vba-lint.ts` に VBA001〜VBA010 として実装済み。`getDiagnostics()` に統合済み。 | `tests/spec/vba-lint.test.ts`

  **実装済みルール (VBA001〜VBA010):**
  ...
  | VBA009 ✅ | デッドストア（代入値が使用前に上書き／破棄される） | Warning |
  | VBA010 ✅ | 到達不能コード（`Exit Sub` / `GoTo` 後のコード） | Information |

  - [ ]: BUG: VBA003 が必ずLn 1, Col1 をエラー箇所として指摘する
  - [ ]: VBA002 デフォルト非表示
  - [ ]: VBA003 デフォルト非表示
  - [ ]: VBA005 デフォルト非表示
  - [ ]: VBA006 デフォルト非表示
  ...
  - [ ]: VBA009 Dim → 初期値設定 → Select Case の Case xxx しかないのにVBA009になる
  - [ ]: VBA010 On Erro Goto LABEL のLABELジャンプ先のコードが到達不能と誤判定されている
...
- [ ] **安全な部分抽出支援（Extract Sub/Function）**
  ...
  **未実装（残タスク）:**
  ...
  - **extension.ts 接続**: `vba-runner.doExtractFunction` コマンドハンドラーで実際のテキスト編集（抽出後の呼び出し挿入 + 新 Sub 挿入）を実装
- [x] **✅ コールグラフの可視化**
...
AI（Claude Code 等）からの利用は、拡張機能が提供する **MCP サーバー** 経由で行う。
リポジトリを clone せずに AI が `analyze` や `run_tests` を呼び出せる。
...
│
└── MCP Server（新規）            AI からツールとして呼び出せる口
    ├── vba_run_tests
    ├── vba_analyze
    ├── vba_get_outline
    └── vba_get_diagnostics
...
拡張機能が MCP サーバーを内蔵し、Claude Code 等の AI がツールとして直接呼び出せるようにする。
VS Code 1.99+ の `contributes.mcpServers` で拡張機能から MCP サーバーを提供できる。

**AI が使えるツール:**

| ツール | 引数 | 戻り値 |
|---|---|---|
| `vba_run_tests` | `path: string` | テスト結果（pass/fail 件数・失敗詳細） |
| `vba_analyze` | `path: string, format?: 'text'\|'json'` | コードスメルレポート |
| `vba_get_outline` | `path: string` | Workspace Outline（AI 向けコンテキスト圧縮） |
| `vba_get_diagnostics` | `uri: string` | VBA001〜VBA010 等の lint 診断結果 |
| `vba_format` | `uri: string, fix?: boolean` | フォーマット差分（`fix: true` で自動適用） |

**AI によるリファクタリングサイクルのイメージ:**

```
1. vba_get_outline(./src/vba/)    → 全体構造を把握（トークン節約）
2. vba_analyze(./src/vba/)        → コードスメルを把握
3. vba_run_tests(./tests/)        → リファクタリング前のテストベースラインを確認
   ... コードを変更 ...
4. vba_format(file.bas, fix=true) → コード整形を自動適用
5. vba_run_tests(./tests/)        → テストがまだ通るか確認
6. vba_analyze(./src/vba/)        → 指標が改善したか確認
```

リポジトリを clone せず、拡張機能をインストールするだけで Claude Code がこのサイクルを回せる。
...
```
src/mcp/
  server.ts      MCP サーバーエントリーポイント（stdio transport）
  tools/
    run-tests.ts   vba_run_tests の実装
    analyze.ts     vba_analyze の実装
    outline.ts     vba_get_outline の実装
    diagnostics.ts vba_get_diagnostics の実装
    format.ts      vba_format の実装（src/lsp/formatter.ts を呼ぶ）
```
...
VBA 実行・テスト実行・MCP 呼び出しはすべて **`child_process`** に分離する。
現在の `Evaluator` には実行時間・メモリ上限がないため、分離は必須要件。

```
Extension Host
├── LSP / Diagnostics / 補完（軽量・同期）← 現状のまま
└── VBA 実行 / テスト実行 / Analyzer / MCP
...
| **MCP の VS Code バージョン要件** | `contributes.mcpServers` は VS Code 1.99+。それ以前はコマンド経由のフォールバックが必要 |
...
**フェーズ 2 — MCP サーバー**
- [ ] `src/mcp/server.ts` 作成（stdio transport で MCP プロトコルを実装）
- [ ] `vba_run_tests` ツール実装
- [ ] `vba_analyze` ツール実装（`test-libs/vba-analyzer.ts` のロジックを移植）
- [ ] `vba_get_outline` ツール実装
- [ ] `vba_get_diagnostics` ツール実装
- [ ] `vba_format` ツール実装（`src/lsp/formatter.ts` を呼ぶ。`fix: true` で TextEdit をファイルに書き戻す）
- [ ] `package.json` の `contributes.mcpServers` に登録
