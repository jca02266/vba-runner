...
- [ ] 標準ファイルI/Oの文字コード処理を共通化する（`Print`/`Write`/`Line Input`/`Input` がそれぞれ独自にバイト変換・デコードしていたため、評価 #168 でCP932マルチバイト文字の扱いが分岐した。TextStreamと同じエンコード境界を共有し、モード別のANSI/Unicode・位置更新を一箇所で管理する）
