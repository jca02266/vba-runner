...
- [ ] 名前付き引数の正規化を全呼び出しAPIへ統合する（クラスProperty・修飾標準モジュール・CallByNameのクラス手続きは共通の式／l-value保持へ段階移行済み。直接の手続きとCallByNameの外部Object/COM経路には別の束縛実装が残るため、同じ正規化フレームへ統合し、Property／標準モジュール／外部呼び出しの全経路でByRef結果を一貫して保持する）
...
- [ ] 既定Valueオブジェクトの型強制を共通化する（`CStr`/`CBool`/数値変換/`IsNumeric`/`IsDate`/`CDate`/`DateValue`/日付関数/`Format` は `__vbaDefault__` のValue展開を実装済みだが、coerce経路と組み込み登録に個別判定が残る。既定Valueだけを一度展開する共通ヘルパーを全経路へ統合し、Object拒否とExcel互換のValue文脈を両立する）
- [ ] 標準ファイルI/Oの文字コード・レコード処理を共通化する（`Print`/`Write`/`Line Input`/`Input` がそれぞれ独自にバイト変換・デコードしていたため、評価 #168〜#169 でCP932マルチバイト文字とInputの行境界処理に分岐が見つかった。TextStreamと同じエンコード境界を共有し、モード別のANSI/Unicode・レコード消費・位置更新を一箇所で管理する）
