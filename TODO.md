...
- [ ] **`MockApplication` と `setDefaultBindingObject` による Tier 6 名前解決**

  MS-VBAL §5.6.10 Tier 6（参照プロジェクト内 Public メンバー）を実装し、
  `Range("A1")` / `Cells(1,1)` / `ActiveSheet` といった修飾なし Excel グローバル呼び出しを
  エンジン上で動かせるようにする。

  **背景**: §5.6.10 の部分修正（`tier6-namespace.test.ts`）により、クラスモジュール名が
  `OBJECT_REQUIRED` ではなく `SUB_OR_FUNCTION_NOT_DEFINED` を返すよう修正済み。
  本タスクはそれに続く「Tier 6 として実際に名前を解決できる仕組み」の実装。
  設計の詳細は `docs/internals/NAME_RESOLUTION.md` §6.5 参照。

  **設計方針**:
  - `Evaluator.setDefaultBindingObject(obj: any)` を追加。`evaluateCallExpression` の末尾で、
    Tier 1〜4 で解決できなかった名前をこのオブジェクトのプロパティ検索にフォールバックする
  - `src/engine/mock/MockExcel.ts`（新設）に `MockApplication` を実装:
    - `Range(address)` → `this.activeSheet.Range(address)`
    - `Cells(row, col)` → `this.activeSheet.Cells(row, col)`
    - `ActiveSheet` プロパティ（`MockWorksheet` インスタンス）
    - `Sheets(nameOrIndex)` → 対応する `MockWorksheet`
    - `MockWorksheet.ts` の既存 `MockApplication` をベースに拡張または統合
  - `MockApplication` は **opt-in**。登録しなければ現在の動作と同じ（未解決名は error 35）
  ...
  **使用イメージ:**
  ...
  // VBA: Range("A1")  → app.Range("A1")  → ActiveSheet.Range("A1")
  // VBA: Cells(1, 1) → app.Cells(1, 1) → ActiveSheet.Cells(1, 1)
  ```

  **作業順序:**
  - [ ] `Evaluator.setDefaultBindingObject(obj)` を追加し、`evaluateCallExpression` に Tier 6 フォールバックを追加
  - [ ] `src/engine/mock/MockExcel.ts` に `MockApplication` を実装（`Range` / `Cells` / `ActiveSheet` / `Sheets`）
  - [ ] `tests/spec/tier6-namespace.test.ts` に Tier 6 動作テスト（`setDefaultBindingObject` 経由）を追加
  - [ ] `docs/internals/NAME_RESOLUTION.md` §6.5 の「計画中」を「実装済み」に更新
  - [ ] `USE_MOCKS.md` に `MockApplication` の使い方を追記
