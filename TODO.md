...
- ✅ **循環参照時の `Set = Nothing` 挙動**: 強制クリアと Class_Terminate の呼び出し順 | テスト: `circular-reference-terminate.test.ts`, `circular-reference-terminate-vba.test.ts` (VBA: `TestHelper.cls`, `CircularRefA.cls`, `CircularRefB.cls`, `Test_circular_reference_terminate.vba`)
