...
- [ ] 多次元オブジェクト配列の添字解決を `Let` / `Set` / `Member` / `With` の共通ヘルパーへ統合する（今回のローカル配列では `Set a(i,j)` と `a(i,j).Member = ...` の経路差が配列状態を壊した。添字数・各次元境界の検証と最終要素への参照取得を一箇所に集約し、宣言場所に依存しない配列要素操作へ段階移行する）
