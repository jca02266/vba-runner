...
`SpyRecord` は `test-libs/test-runner` からはエクスポートされていない。型が必要な場合は `src/engine/evaluator` から直接インポートする。

```typescript
import { VBARunner } from '../../test-libs/test-runner';
import type { SpyRecord } from '../../src/engine/evaluator';
```
...
| `assert.notStrictEqual(actual, expected, msg?)` | `!==` で比較。等しければ失敗 |
...
| `assert.throws(fn, msg?)` | 関数が例外を投げることを検証 |
| `assert.throwsMatch(fn, pattern, msg?)` | 例外メッセージが文字列または RegExp にマッチすることを検証 |
| `assert.doesNotThrow(fn, msg?)` | 関数が例外を投げないことを検証 |
