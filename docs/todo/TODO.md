...
- [x] **DAP（デバッガー）の VS Code 接続**
  - [x] ステップ実行・ブレークポイント・変数表示・実行時エラー位置通知を実装。テスト: `lsp-debug-adapter.test.ts`, `lsp-debug-hook.test.ts`, `lsp-debug-session.test.ts`
...
- [x] **DAP ステップ実行の本実装**
  ...
  **実装済み:**
  ...
  - テスト: `lsp-debug-hook.test.ts`（7件）、`lsp-debug-session.test.ts`（11件）
