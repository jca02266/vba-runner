...
type: Roadmap
title: VBA開発環境改善TODO
description: VBA仕様準拠以外の開発環境、テスト支援、モック、解析器の改善項目。
tags: [vba, roadmap, development]
status: active
---
...
組み合わせた開発支援環境を目指しています。このTODOでは、VS Code拡張機能を
含む各パッケージとツール群を対象に、特定のエディターに依存しないVBA開発支援
環境全体の改善を管理します。
...
- [x] **`__mocks__` ディレクトリによるモック注入** ✅ 実装済み | `tests/test-libs-tests/mock-loader.test.ts`
  ...
  **名前解決の優先順序（`▶ Run` 実行時）:**
  ...
  ### [ ] __mocks__ ひな形自動生成（vba-runner.generateMocks コマンド）

  **前提：サイレント注入と __mocks__ の関係**

  `injectExcelStub`（サイレント注入）は **常時有効なベースライン** であり、
  撤廃しない。`__mocks__/` に書いたファイルは `injectExcelStub` より後にロードされ、
  env が Tier 6（defaultBindingObject）より先に検索されるため、
  **`__mocks__/` の定義がサイレント注入を部分的に上書きできる**。

  つまり現状でも「動かす → 気に入らない挙動だけ `__mocks__/` に書き足す」フローは
  すでに成立している。

  **ひな形生成が解決する課題:**

  ユーザーが「どう書けばいいかわからない」段階で手が止まる。
  `__mocks__/` の書き方を知らなくても、コマンドひとつで「このファイルの Excel 依存箇所を
  差し替えるならこういう .bas を書けばよい」という出発点を得られるようにする。

  **想定フロー:**

  1. `▶ Run` でひとまず動く（`injectExcelStub` のサイレント注入で）
  2. 「セル A1 の値を 100 にして動かしたい」などカスタマイズしたくなる
  3. Code Lens の「Generate Mock Stub」ボタンをクリック（または `VBA: Generate Mock Stub` コマンド）
  4. そのプロシージャが使う Excel オブジェクトだけを対象にした `.bas` ひな形が
     `__mocks__/ExcelObjects.bas` に生成される
  5. ユーザーがファイルを編集してセル値や戻り値を調整
  6. 再実行 → `__mocks__/` の定義がサイレント注入を上書きして意図した挙動になる

  **設計方針:**

  - **全部一気に生成しない** — カーソル位置のプロシージャが使う Excel オブジェクトだけを対象にする
    (`excelMockTargets[].objects` の集合を見る)
  - **生成形式は `.bas`** — VBA として読み書きできる。`.js` より敷居が低い
  - **既存ファイルは上書きしない** — `__mocks__/ExcelObjects.bas` が既にある場合は
    不足しているクラス・関数だけを Output Panel に表示して手動追記を促す
  - **スタブは最小限** — クラスのフィールドは空、メソッドは `0` や `""` を返すだけ。
    ユーザーが必要な箇所だけ書き足す設計

  **生成例（`AutoScheduleTasks` の場合）:**

  ```vba
  ' __mocks__/ExcelObjects.bas
  ' 自動生成: vba-runner.generateMocks (2026-06-06)
  ' 必要に応じてセル値・戻り値を書き換えてください

  Class MockWorksheet
      Public Function Cells(row, col)
          Set Cells = New MockRange
      End Function
  ...
  Function Sheets(nameOrIndex)
      Set Sheets = New MockWorksheet
  End Function
  ```

  **実装作業順序（着手時）:**
  - [ ] `vba-runner.generateMocks` コマンドを extension.ts に登録
  - [ ] アクティブファイルのパスを受け取り、同ディレクトリで `vba-analyzer --json` を実行
  - [ ] `excelMockTargets[].objects` の和集合から必要クラス・関数のセットを決定
  - [ ] `.bas` テンプレート文字列を組み立てて `__mocks__/ExcelObjects.bas` に書き出す
  - [ ] 既存ファイルがある場合は差分（未定義クラスのみ）を Output Panel に表示
  - [ ] Code Lens に `Generate Mocks` ボタンを追加（Excel 依存ありのプロシージャのみ）
  - [ ] `injectExcelStub` を「`__mocks__` 非存在時のフォールバック」に格下げ
  ...
  **背景・現状の問題:**
  ...
  現在の `MockWorksheet` / `MockRange` は書き込みが読み取りに反映される **フェイク（Fake）** 実装になっている。
  例: `ws.Cells(1,1).Value = 100` → `ws.Cells(1,1).Value` を読むと `100` が返る。

  テスト理論では以下の3種類が区別される:

  | 種類 | 入力（書き込み） | 出力（読み取り） | 呼び出し検証 |
  |---|---|---|---|
  | **スタブ（Stub）** | 無視（副作用なし） | テストが事前設定した値 | ❌ |
  | **モック（Mock）** | 無視（副作用なし）+ 記録 | テストが事前設定した値 | ✅（何回・何で呼ばれたか） |
  | **フェイク（Fake）** | 内部状態に反映 | 書き込んだ値が読める | ❌ |

  現状の `MockWorksheet` はフェイクだが、名前が "Mock" になっている。

  **理想の設計:**

  ```
  デフォルト（スタブモード）:
    ws.Cells(1,1).Value = 100  → 副作用なし（書き込みはスパイに記録されるだけ）
    ws.Cells(1,1).Value        → テストが事前に設定した値を返す（例: 42）
    spy.assertWritten('A1', 100)  → 書き込みを検証できる

  オプション（フェイクモード / 現状の挙動）:
    ws.Cells(1,1).Value = 100  → 内部状態に反映
    ws.Cells(1,1).Value        → 100 が返る
  ```

  **メリット:**

  - スタブモードでは VBA コードが何を書き込もうと読み取り値は変わらない
    → テストの入力（前提条件）と出力（検証対象）が明確に分離される
  - モックとして `spy.assertWritten('A1', result)` で書き込み内容を検証できる
  - フェイクモードはオプションとして残す（統合テスト的な使い方に有用）

  **現実的かどうか:**

  はい、現実的。実装方針としては:
  - `MockRange` に `mode: 'stub' | 'fake'` を持たせる（デフォルト `'stub'`）
  - stub モードでは `Value` setter が書き込みをスパイに記録するだけ
  - stub モードでは `Value` getter がコンストラクタ / `setReturn()` で渡した値を返す
  - `MockWorksheet.setReturn('A1', 42)` でセルごとに戻り値を設定できる
  - `MockWorksheet.assertWritten('A1', 100)` で VBA が書いた値を検証できる
  - フェイクモードは `new MockWorksheet({ mode: 'fake' })` で明示的に選択

  **影響範囲:**

  - `src/engine/mock/MockWorksheet.ts`（MockRange / MockWorksheet の設計変更）
  - `injectExcelStub` / `injectAccessStub` / `injectWordStub`（デフォルトをスタブモードに）
  - `docs/MOCK_GUIDE.md`（Part 0、モック・スタブの使い分けを説明）
  - 既存テストの更新（フェイク前提のテストはモードを明示）
  ...
  **作業順序（実施時）:**
  - [x] Evaluator に `builtinOverrides` オプションを追加（`setBuiltinOverride` / `getRegisteredClassNames` / `promoteMockVbaClasses` を追加）
  - [x] VBA モック評価後にクラスを `externalObjectFactories`・関数を `builtinOverrides` へ登録する仕組みを追加
  - [x] `test-libs/mock-loader.ts` に `loadMocks(dir: string, evaluator: Evaluator): MockModule[]` を実装（`__mocks__/` と `__mocks__.*` を自動検出し `.ts`・`.js`・`.bas` をそれぞれロード）
  - [x] `VBARunner` のコンストラクターで `loadMocks` を呼び出す
  - [x] extension.ts の `runProcedure` でも `loadMocks` を呼び出す

- [x] **`MockApplication` と `setDefaultBindingObject` による Tier 6 名前解決** ✅ 実装済み

  MS-VBAL §5.6.10 Tier 6（参照プロジェクト内 Public メンバー）を実装。
  `Range("A1")` / `Cells(1,1)` / `ActiveSheet` / `Sheets("name")` などの
  修飾なし Excel グローバル呼び出しをエンジン上で動かせるようになった。

  **実装済み:**
  - `Evaluator.setDefaultBindingObject(obj)` — Tier 6 フォールバック先を登録
  - `src/engine/mock/MockExcel.ts` — `MockApplication` を実装（`Range` / `Cells` / `ActiveSheet` / `Sheets` / `ActiveWorkbook` / `ThisWorkbook`）
  - `MockWorksheet.Cells(row, col)` メソッドを追加
  - テスト: `tests/spec/tier6-namespace.test.ts`（テスト 7〜12）
  - `docs/internals/NAME_RESOLUTION.md` §6.5 を「実装済み」に更新
  ...
  **Option Explicit に関する注意**: `Range` / `ActiveSheet` 等は宣言なしの識別子として
  使うため、これらを使う VBA モジュールは `Option Explicit` なしにする必要がある。
  実 Excel VBA でも同様（これらは型ライブラリ経由で解決されるため宣言は不要）。

- [x] **Signature Help（引数入力支援）**（ユーザー定義プロシージャ・組み込みVBA関数を実装）
  関数呼び出しの `(` 内を入力中に、現在カーソルがある引数の名前・型・説明をツールチップ表示する。
  VBA エディター（VBE）の「クイックヒント」相当の機能。キーワード引数（`name:=value` 形式）の入力補助にも使える。

  **実装範囲:**

  - `(` 内の未閉じ呼び出しを検出し、カンマ区切りからアクティブな引数位置を計算
  - ユーザー定義 Sub/Function の引数名・型と、組み込みVBA関数の静的シグネチャを表示
  - キーワード引数の型候補やExcel/COM型ライブラリの完全な引数定義は未対応
  ...
  **今後の拡張:**
  ...
  1. キーワード引数（`name:=value`）から対応する引数を選択
  2. Excel/COM型ライブラリから引数定義をロード
  3. 組み込み関数の静的シグネチャを段階的に拡充
...
- [x] **VBA 固有の落とし穴に対する Diagnostics 警告** — `src/engine/vba-lint.ts` に VBA001〜VBA012 として実装済み。`getDiagnostics()` に統合済み。 | `tests/vba-lint.test.ts`
...
- [x] **Excel API スタブモード（実行系の拡張）** ✅ 実装済み | `tests/test-libs-tests/excel-stub.test.ts`
  - `VBARunner('dir', { excelStub: true })` で MockApplication を自動注入
  - `injectExcelStub(evaluator)` 関数（`test-libs/excel-stub.ts`）で任意の Evaluator に適用可能
  - 対応: `ActiveSheet` / `Range` / `Cells` / `Sheets` / `Application.ScreenUpdating` 等 / `Rows.Count` / `Columns.Count` / `End(dir).Row` 等
  - extension.ts の `▶ Run` でも自動注入（`__mocks__` で上書き可能）
  - `VBARunner.excelStub` プロパティで事前セルデータを設定できる
...
### 直近の互換性改善実施計画（2026-07-29）

次の順序で、回帰確認・実Excel照合・共通化を進める。各段階で結果を評価記録と
回帰テストへ反映し、修正がある場合は個別コミットする。

- [x] `scripts/bg-test.sh` を実行し、TypeScript/VBAテストの成功・失敗・未実施件数と所要時間を記録する（初回: 333/333、14/14、111/111、未実施0、約2分。共通化後の再実行: 333/333、14/14、111/111、未実施0、約8分。XL-016修正後の再実行: 333/333、14/14、111/111、未実施0、約7分。数値リテラル共通化後の再実行: 333/333、14/14、111/111、未実施0、約7分。Select Case比較共通化後の再実行: 333/333、14/14、111/111、未実施0、約7分。Err状態共通化後の再実行: 333/333、14/14、111/111、未実施0、約7分。ByVal/ByRef共通化後の再実行: 333/333、14/14、111/111、未実施0、約7分。配列添字共通化後の再実行: 333/333、14/14、111/111、未実施0、約7分。既定Value共通化後の再実行: 333/333、14/14、111/111、未実施0、約7分。標準テキストI/O共通化後の最終再実行: 333/333、14/14、111/111、未実施0、約7分。途中の評価件数固定値の失敗は`8c61f35`および`3bef0d6`で修正。修正前の未使用import失敗は`64db649`で修正）
- [ ] bg-test結果を基に、未照合の実Excel候補を整理し、Windows Excelでまとめて照合する（`tests/excel/queue/ExcelQueueVerification.bas` にXL-013〜XL-017を追加済み。Windows Excel実行待ち）
...
新方式では `evaluation/states/*.events.yml` の `occurredAt` を時系列集計の正本とする。
完了日時のない旧評価は、評価本文・旧 `EVAL_LOG.md`・状態スナップショット・Git履歴を
突き合わせ、状態遷移を裏付けるコミット日時が特定できる場合はイベントへ移行する。
コミット日時が評価完了を直接示さない場合は推測で補完せず、日時未確定として残す。
`completedAt` は旧形式との互換用に限り、イベントの代替として新規追加しない。

- [x] EV-00100をサンプルとしてGitコミット `2a4dac6`（2026-07-26T09:21:02+09:00）から `verified-no-bug` の状態イベントを復元し、レポートへ反映する
- [x] 完了日時なしのEV-00410をGitコミット `98470f5`（2026-08-03T23:35:55+09:00）から `verified-no-bug` の状態イベントへ移行し、レポートへ反映する
- [x] 残りの完了日時なし `verified-no-bug`（EV-00186、EV-00187、EV-00188、EV-00412）を各検証コミットから状態イベントへ移行し、レポートへ反映する
- [x] 完了日時もイベントもなかった固定済みEV-00180〜EV-00185、EV-00400、EV-00406、EV-00408、EV-00409、EV-00411、EV-00413、EV-00414をGit履歴から状態イベントへ移行し、レポートへ反映する
- [x] 完了日時もイベントもなかったknown-limitのEV-00407をGitコミット `3942fa7`（2026-08-04T20:46:31+09:00）から状態イベントへ移行し、レポートへ反映する
- [ ] 完了日時のない旧評価を一覧化し、Git履歴に状態遷移の根拠があるものを候補化する
- [ ] 候補ごとに状態・評価ID・コミット根拠をレビューし、承認済みのイベントだけを追加する
- [ ] 移行後に `npm run eval -- validate` と `scripts/eval-report.mjs` を実行し、時系列件数とグラフを確認する
...
### VBA 呼び出しフレームと ByRef 参照処理の統合

**問題**: 標準モジュール、クラスメソッド、Property Set/Let、配列要素、UDTメンバーで
呼び出し・引数束縛・ByRef書き戻しの経路が分かれている。現在は呼び出し元の環境
（モジュール環境、クラスの `instanceEnv`、手続きローカル環境）によって処理が異なり、
同じ `ByRef` 引数でも書き戻しの有無が変わるバグが発生しやすい。評価 #130〜#132 で、
宣言サフィックス付き引数、クラス配列要素、Property Set の各経路に実例が見つかった。

**目標**: 変数の定義場所や呼び出し形式に依存せず、VBAの `ByRef` を一貫して扱う。
...
- [ ] 名前付き引数の正規化を全呼び出しAPIへ統合する（クラスProperty・修飾標準モジュール・CallByNameのクラス手続きは共通の式／l-value保持へ段階移行済み。外部Object/COMのprototypeメンバー解決と位置・Optional・添字・オブジェクト引数は確認済みだが、外部スカラーByRefは値渡し制限として残る。外部CallByNameの名前付き引数はParamArrayに外部引数名がないためError 448となる仕様境界を維持しつつ、可能な外部呼び出しの束縛を共通化する）
...
- [ ] 既定Valueオブジェクトの型強制を共通化する（`CStr`/`CBool`/数値変換/`IsNumeric`/`IsDate`/`CDate`/`DateValue`/日付関数/`Format` と #178/#185 のNull処理は `__vbaDefault__` のValue展開と循環拒否を実装済みだが、coerce経路と組み込み登録に個別判定が残る。既定Valueだけを一度展開する共通ヘルパーを全経路へ統合し、Object拒否とExcel互換のValue文脈を両立する）
...
以下はVBA仕様準拠そのものではなく、テスト支援・外部連携・解析機能の拡張課題である。

### テストダブル・外部連携

- **Spy / Verify 機能**: `Shell` や `MsgBox` 等のスタブ・モックに渡された引数をテストコードから検証する仕組み。現状は `Debug.Print` 相当の出力確認のみ。
- **日付・時刻の仮想化（`Date` / `Now` / `Time`）**: テスト実行時に固定日時を注入し、再現性を高める仕組み。現状はシステム時刻を返す。
- **外部COMオブジェクトのスタブ・フェイク拡充**: `MSXML2.XMLHTTP` 等の頻出オブジェクトをテスト用に提供する。

### vba-analyzerの高度解析

現在の `vba-analyzer` はRange変数経由のアクセス検出などを実装している。
CFG / reaching-defs / live-vars の基盤を活用した次の機能は将来検討とする。
