...
- ✅ **Fix: `Rnd(-1)`がExcelと異なる値・後続系列を返す** | `math-module.test.ts`
  - 原因: 負数引数が独自の式で`lastRnd`だけを更新し、正数・無引数呼び出しが使う系列状態を更新していなかった
  - 症状: Excelの`0.224007 → 0.03584582`に対し、runnerは`0.2511917009602195 → 0.000614211312495172`を返した
  - 修正: VBA互換の24ビット線形合同法とSingleのビット表現による負数シードを使い、Rndの全引数区分で一つの系列状態を共有する
