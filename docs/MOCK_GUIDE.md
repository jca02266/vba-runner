...
// セルの値を読み書き
Value: any          // get/set
__vbaDefault__: true  // Let 代入時に valueOf() を呼ぶ opt-in マーカー
valueOf(): any      // Value を返す（スカラーコンテキスト用）
```
...
```typescript
class MockRange {
  readonly __vbaDefault__ = true;  // スカラーコンテキスト opt-in
  ...
  valueOf() { return this.value; }  // x = rng で Value が抽出される
}
...
- **デフォルトメンバー** → スカラーコンテキスト（`v = rng`）では VBA Runner が `valueOf()` を呼んで値を抽出する。ただし opt-in が必要（下記参照）。`Set v = rng` のようなオブジェクトコンテキストではオブジェクト自体が代入されるため変換は起きない

#### スカラーコンテキストの opt-in: `__vbaDefault__`

`v = rng`（Let 代入）で `.Value` が自動抽出されるには、モックオブジェクトに以下の 2 つを実装する必要があります：

```typescript
class MockRange {
    readonly __vbaDefault__ = true;  // opt-in マーカー

    constructor(private value: any) {}

    get Value(): any { return this.value; }
    set Value(val: any) { this.value = val; }

    valueOf(): any { return this.value; }  // Let 代入時に呼ばれる
}
```
