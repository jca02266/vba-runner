...
### R-12: Functional Core / Imperative Shell（FC/IS）

副作用がロジックの途中に散在しているが、**副作用の実行タイミングを後ろにまとめても結果が変わらない**場合に適用する。副作用をすべて末尾（Shell）に移動し、それより前を副作用なしの純粋ロジック（Core）として切り出す。Core は [T-01](#t-01) で直接テストできる。
...
### R-13: Seam（接合部）による副作用の抽出

*Michael Feathers「レガシーコード改善ガイド」より*

**副作用が混在していてテストが書けない状態を、テストが書ける状態に変えるための前段階として行うリファクタリング手法。** 副作用の「塊」を関数抽出（[R-01](#r-01)）で関数として抽出し、テスト時にモックに差し替えられる差し替えポイント（＝接合部）を作る。

以下のいずれかの条件が当てはまる場合に適用する（[R-12](#r-12) FC/IS が使えないケース）。

* 副作用の出力が後続のロジック分岐に影響する
* 副作用への入力値が処理途中の計算結果に依存する（引数に追い出しても呼び出し元に問題が移るだけ）
...
### R-14: マクロ記録コードのレシーバー明示化

Excel の「マクロの記録」機能が生成するコードは `ActiveSheet`・`ActiveCell`・`Selection` などの現在の選択状態に依存する。実行中にユーザーが別のシートやセルをクリックするだけで意図しない箇所が操作されてしまう。

**問題のあるパターンと修正方針:**

```vb
' NG: 暗黙レシーバー（マクロ記録が生成するコード）
Sub BadMacro()
    Range("A1").Value = 100          ' 実行時のアクティブシートに依存
    ActiveSheet.Name = "Result"      ' 実行時のアクティブシート依存
    Selection.Font.Bold = True       ' 選択したセルの位置に依存
    Cells(1, 1).Interior.Color = vbYellow  ' 実行時のアクティブシートに依存
End Sub

' OK: レシーバー(ws)を明示
Sub GoodMacro(ws As Worksheet)
    ws.Range("A1").Value = 100
    ws.Name = "Result"
    ws.Range("A1").Font.Bold = True
    ws.Cells(1, 1).Interior.Color = vbYellow
End Sub
```

レシーバーを明示することで：

* ユーザーの操作（別シートのクリック等）に影響されなくなる

* 複数シートを対象とするループ処理でも同じ関数を再利用できる

**`ActiveWorkbook`** **→** **`ThisWorkbook`** **への置き換え:**

`ActiveWorkbook` はユーザーが別のブックをクリックした瞬間に切り替わる。マクロが含まれているブック自身を指す場合は `ThisWorkbook` で固定できる。

```vb
' NG: 実行中に別ブックをアクティブにすると壊れる
Sub BadWorkbook()
    ActiveWorkbook.Sheets("Config").Range("A1").Value = 100
    ActiveWorkbook.Save
End Sub

' OK: マクロを含むブック自身を明示
Sub GoodWorkbook()
    ThisWorkbook.Sheets("Config").Range("A1").Value = 100
    ThisWorkbook.Save
End Sub
```

ただし `ThisWorkbook` は「マクロが記述されているブック」を指すため、別のブックを操作したい場合は適切ではない。その場合は `Workbooks("ファイル名.xlsx")` で名前指定するか、引数 `wb As Workbook` で受け取る設計にする。

```vb
' 別ブックを操作する場合は引数で受け取る
Sub ProcessWorkbook(wb As Workbook, ws As Worksheet)
    ws.Range("A1").Value = wb.Name
End Sub
```

**検出**: `vba-analyzer` の `excelAccessCount`・`excelMockTargets` で `ActiveSheet`・`Selection` を含む関数を特定できる（[VA-01](#va-01)）。

***

#### ⚠️ PasteSpecial に依存するコードは単純な明示化では解決しない

マクロ記録が生成する `Copy` + `PasteSpecial` パターンはクリップボードを経由する。
レシーバーを明示しても**クリップボード依存は残る**ため、根本的な書き換えが必要。

```vb
' NG: クリップボード依存（他アプリの操作・ユーザーの Ctrl+C で壊れる）
Range("A1:A10").Copy
Range("B1").PasteSpecial Paste:=xlPasteValues
Application.CutCopyMode = False  ' ← これはクリップボードをクリアするだけで根本解決ではない
...
' OK: 値と数値書式 → ループで個別コピー（PasteSpecial xlPasteValuesAndNumberFormats の代替）
...
**`PasteSpecial`** **オプション別の代替手段:**

| PasteSpecial のオプション             | 代替手段                                             |
| ------------------------------- | ------------------------------------------------ |
| `xlPasteValues`                 | `.Value = src.Value`                             |
| `xlPasteFormulas`               | `.Formula = src.Formula`                         |
| `xlPasteFormats`                | ループで `.NumberFormat`・`.Interior`・`.Font` 等を個別コピー |
| `xlPasteValuesAndNumberFormats` | `.Value` + `.NumberFormat` の組み合わせ                |
| `xlPasteAll`（書式含む完全コピー）         | 代替困難。クリップボード経由を維持しつつレシーバーを明示する妥協案も検討             |
| `xlPasteColumnWidths`           | `.ColumnWidth = src.ColumnWidth`（列ループ）           |

`xlPasteAll` など書式の完全コピーは直接代替が難しく、クリップボード経由を残さざるを得ない場合がある。その際は操作を最小スコープに限定し、Copy〜PasteSpecial の間にユーザー操作が入らないよう `Application.ScreenUpdating = False` と組み合わせるか、処理を専用 Sub に切り出してユーザーに「実行中は操作しないよう」明示すること。

***

#### TODO: ツール支援

* [ ] `vba-analyzer` で `ActiveSheet`・`ActiveCell`・`Selection` を使っている箇所を一覧化し、`excelMockTargets` に含める
...
### R-15: Collection Pipeline パターンの明示化と分離

*Martin Fowler "Collection Pipeline" より*

`filter`・`map`・`reduce` の組み合わせで表現できるロジックを **Collection Pipeline** と呼ぶ。VBA のループコードはこれらが混在して意図が埋もれやすい。**コメント注釈による意図の明示**と**操作単位への関数分離**の2段階で改善する。

***

**各操作の対応関係:**

| 操作         | 意味          | LINQ        | Java Stream | SQL                |
| ---------- | ----------- | ----------- | ----------- | ------------------ |
| **filter** | 条件で絞り込む     | `Where`     | `filter`    | `WHERE`            |
| **map**    | 要素を別の値に変換する | `Select`    | `map`       | `SELECT 列`         |
| **reduce** | 集約して1つの値にする | `Aggregate` | `reduce`    | `SUM` / `GROUP BY` |

***

**Step 1: コメント注釈で意図を明示する（即効性が高い）**

ループの先頭に操作名を宣言するだけで可読性と AI 可読性が大きく向上する。

```vb
' FILTER: ステータスが "Active" の行のみ対象
For i = 1 To lastRow
    If ws.Cells(i, COL_STATUS).Value = "Active" Then
        ...
    End If
Next i
...
For i = 1 To UBound(amounts)
    display(i) = Format(amounts(i), "#,##0")
Next i

' REDUCE: 部門ごとの合計を集計
total = total + ws.Cells(i, COL_AMOUNT).Value

' GROUP BY: 部門 → Dictionary でグループ別集計  （reduce の特殊形）
If Not groups.Exists(dept) Then groups(dept) = 0
groups(dept) = groups(dept) + amount
```

***

**Step 2: 操作ごとに関数に分離する（テスト・再利用のため）**

filter と map が混在したループは、それぞれ独立した純粋関数（[R-01](#r-01)）に切り出すことで個別にテスト・再利用できる。

```vb
' Before: filter と map が混在 → テストしにくく再利用もできない
Function GetActiveDisplayAmounts(ws As Worksheet, lastRow As Long) As String()
    Dim result() As String
    Dim count As Long
    For i = 1 To lastRow
        If ws.Cells(i, COL_STATUS).Value = "Active" Then   ' filter
            count = count + 1
            ReDim Preserve result(1 To count)
            result(count) = Format(ws.Cells(i, COL_AMOUNT).Value, "#,##0")  ' map
        End If
    Next i
    GetActiveDisplayAmounts = result
End Function

' After: filter と map を分離
' FILTER: "Active" 行のインデックスを返す
Function FilterActiveRows(ws As Worksheet, lastRow As Long) As Long()
    Dim result() As Long
    Dim count As Long
    Dim i As Long
    For i = 1 To lastRow
        ...
        End If
    Next i
    FilterActiveRows = result
End Function

' MAP: 行インデックスの配列 → 表示用文字列の配列に変換
Function MapToDisplayAmount(ws As Worksheet, rows() As Long) As String()
    Dim result() As String
    ReDim result(1 To UBound(rows))
    Dim i As Long
    For i = 1 To UBound(rows)
        result(i) = Format(ws.Cells(rows(i), COL_AMOUNT).Value, "#,##0")
    Next i
    MapToDisplayAmount = result
End Function

' 呼び出し側
Dim activeRows() As Long
Dim displayAmounts() As String
activeRows    = FilterActiveRows(ws, lastRow)
displayAmounts = MapToDisplayAmount(ws, activeRows)
```
...
| 分離すべきケース                    | 見送りで良いケース              |
| --------------------------- | ---------------------- |
| filter 条件が複雑なビジネスルール        | filter が 1 行の単純な等値比較   |
| 変換ロジックに計算が含まれる              | 変換が `Format()` 1 つで終わる |
| filter か map のどちらかを別の箇所でも使う | 一箇所だけの使い捨てループ          |
| 独立したテストを書きたい                | テストの必要性が低い             |

***

**入出力が Excel シートだけで完結する場合は Excel 関数への置き換えを検討する**

filter・map・reduce の入力も出力も Excel シート上のデータであれば、VBA のループを書かずに **Excel のワークシート関数だけで完結させる**選択肢がある。Excel 365 以降では以下の動的配列関数が利用可能で、数式バーに直接記述できる。

| 操作 | Excel 関数 | 例 |
|---|---|---|
| **filter** | `FILTER` | `=FILTER(A2:A100, B2:B100="Active")` — B列が "Active" の行だけ抽出 |
| **map** | `MAP` | `=MAP(A2:A10, LAMBDA(x, x*1.1))` — 各要素に 1.1 を掛ける |
| **reduce** | `REDUCE` | `=REDUCE(0, A2:A10, LAMBDA(acc,x, acc+x))` — 合計（SUM の手書き版） |
| **filter + reduce** | `SUMPRODUCT` | `=SUMPRODUCT((B2:B100="Active")*C2:C100)` — 条件付き合計（旧バージョンでも動く） |
| **map（行ごと）** | `BYROW` | `=BYROW(A2:C10, LAMBDA(row, SUM(row)))` — 行ごとの合計 |
| **scan（累積）** | `SCAN` | `=SCAN(0, A2:A10, LAMBDA(acc,x, acc+x))` — 累積和を配列で返す |

VBA を選ぶ基準と Excel 関数を選ぶ基準:

| VBA が適する場合 | Excel 関数が適する場合 |
|---|---|
| 処理結果を他のシステム・ファイルに渡す | 結果をそのままシートに表示するだけ |
| ロジックが複雑で数式に収まらない | ロジックが単純な条件・変換・集計 |
| ユーザー操作のトリガーで動かしたい | シートの値が変わるたびに自動再計算したい |
| ユニットテストを書きたい | 数式自体がドキュメントとして読みやすい |
| Excel 2016 以前のバージョンが対象 | Excel 365 / 2021 以降が保証されている |

***

**TODO: ツール支援**

* [ ] `vba-analyzer` でネストした `If` を含む `For` ループを検出し、Collection Pipeline パターン候補として報告

* [ ] 同一ループ内に filter・map・reduce 相当の操作が混在している場合に分離候補として提示

* [ ] `FILTER:` / `MAP:` / `REDUCE:` コメントを `vba-analyzer` が読んで意図を構造化データとして出力（AI 連携向け）

***

<a id="r-16"></a>

### R-16: 標準関数不足の自作ユーティリティ化（ライブラリ化）

VBA は標準ライブラリが薄く、他の言語では 1 行で書ける処理が複数行の決まり文句になりやすい。同じ決まり文句が 3 回以上出現したら（DRY 原則）、意味のある名前を持つユーティリティ関数に抽出して専用モジュールにまとめる。
...
* VBA Runner でユーティリティ単体をテストできる（[R-01](#r-01)・[T-01](#t-01)）

* `vba-analyzer` の重複ブロック検出（[VA-01b](#va-01b)）で候補が自動発見できる

***

**カテゴリ別：よく不足する標準関数と自作パターン**

**文字列操作:**

```vb
' NG: 意図が埋もれた決まり文句が各所に散在
If InStr(1, UCase(s), UCase(keyword)) > 0 Then ...
If Trim(s) = "" Or IsNull(s) Then ...
If Left(s, Len(prefix)) = prefix Then ...

' OK: 意図を名前で表現したユーティリティ
If ContainsIgnoreCase(s, keyword) Then ...
If IsNullOrWhitespace(s) Then ...
If StartsWith(s, prefix) Then ...

' LibString.bas に定義
Function ContainsIgnoreCase(s As String, keyword As String) As Boolean
    ContainsIgnoreCase = InStr(1, UCase(s), UCase(keyword)) > 0
End Function

Function IsNullOrWhitespace(s As Variant) As Boolean
    IsNullOrWhitespace = IsNull(s) Or Trim(CStr(s)) = ""
End Function
...
```vb
' NG: IsNumeric チェックが各所に散在
If IsNumeric(val) Then result = CLng(val) Else result = 0

' OK: デフォルト値付き安全変換
result = SafeCLng(val, 0)

Function SafeCLng(val As Variant, defaultVal As Long) As Long
...
Function SafeCDbl(val As Variant, defaultVal As Double) As Double
    If IsNumeric(val) Then
        SafeCDbl = CDbl(val)
    Else
        SafeCDbl = defaultVal
...
' Null/Empty の Coalesce（最初の非 Null 値を返す）
Function Coalesce(val As Variant, defaultVal As Variant) As Variant
...
```vb
' NG: 「配列に含まれるか」チェックが各所でループ
Dim found As Boolean
Dim i As Long
For i = LBound(arr) To UBound(arr)
    If arr(i) = target Then found = True: Exit For
Next i

' OK: 意図が明確
If ArrayContains(arr, target) Then ...

Function ArrayContains(arr As Variant, target As Variant) As Boolean
    Dim i As Long
    For i = LBound(arr) To UBound(arr)
        If arr(i) = target Then ArrayContains = True: Exit Function
    Next i
End Function

Function ArrayIndexOf(arr As Variant, target As Variant) As Long
    Dim i As Long
    For i = LBound(arr) To UBound(arr)
        If arr(i) = target Then ArrayIndexOf = i: Exit Function
    Next i
    ArrayIndexOf = -1  ' 見つからない場合
End Function
```
...
```vb
' NG: Exists チェックが各所に散在
If dict.Exists(key) Then val = dict(key) Else val = 0

' OK
val = DictGetOrDefault(dict, key, 0)

Function DictGetOrDefault(dict As Object, key As Variant, defaultVal As Variant) As Variant
    If dict.Exists(key) Then
        DictGetOrDefault = dict(key)
    Else
        DictGetOrDefault = defaultVal
    End If
End Function
```
...
Function IsWeekend(d As Date) As Boolean
    Dim w As Integer
    w = Weekday(d)
    IsWeekend = (w = vbSaturday Or w = vbSunday)
End Function

Function IsWeekday(d As Date) As Boolean
    IsWeekday = Not IsWeekend(d)
End Function
```
...
```
src/vba/
├── LibString.bas      ← 文字列ユーティリティ
├── LibConvert.bas     ← 型変換ユーティリティ（SafeCLng, SafeCDbl, Coalesce 等）
├── LibArray.bas       ← 配列操作ユーティリティ
├── LibDict.bas        ← Dictionary 操作ユーティリティ
├── LibDate.bas        ← 日付ユーティリティ
└── (業務ロジック).bas
```

1 つのモジュールが肥大化してきたら機能カテゴリごとに分割する。ユーティリティ関数は副作用なしの純粋関数に保つことで、VBA Runner からテスト可能な状態を維持できる。

***

**TODO: ツール支援**

* [ ] `vba-analyzer` の重複ブロック検出（[VA-01b](#va-01b)）で検出したパターンと、既知の「不足しがちな標準関数」パターン（`InStr > 0`・`IsNumeric + CLng` 等）を照合してユーティリティ化を提案

* [ ] `vba-analyzer --json` の `duplicateBlocks` に `suggestedFunctionName` フィールドを追加し、AI が関数定義を生成しやすくする

***

<a id="r-17"></a>

### R-17: Sprout Method（発芽メソッド）

**テストのない既存コードに新機能を追加しなければならない場面**で使う手法。既存コードにはテストがなくリファクタリングできないが、新機能だけは安全にテスト可能な状態で追加したい場合に適用する。既存コードを変更せず、新ロジックを別の Sub/Function に実装して呼び出す。変更箇所を「呼び出しを 1 行追加するだけ」に限定できるため、既存ロジックを壊すリスクが最小になる。
...
切り出した新関数は VBA Runner で独立してテストできる。
...
**テストのない既存コードに前後処理を追加しなければならない場面**で使う手法。既存コードには触れずに横断的関心事（ログ・バリデーション・トランザクション管理等）を追加したい場合に適用する。既存メソッドをリネームして `_Impl` などの名前に移し、同名の新メソッドで旧実装を呼び出す形にする。既存の呼び出し元コードを一切変更しなくてよい点が Sprout Method（[R-17](#r-17)）との違い。
...
実行結果や副作用の呼び出し列を初回に記録し、以降のリファクタリングで「変わらないこと」を確認するテスト手法。2 つの用途がある。

**用途 A: 戻り値スナップショット**
関数の戻り値をそのまま記録し、リファクタリング後に同じ値が返ることを確認する。純粋関数（[R-01](#r-01) / [R-12](#r-12)）に適用しやすい。

```typescript
const result = vbaRunner.run('GenerateReport', []);
expect(result).toMatchSnapshot();  // 初回記録、以降は差分検出
```

**用途 B: 副作用の呼び出し列スナップショット**
副作用を関数に抽出（[R-13](#r-13)）してモック化した後、「どの引数で何回呼ばれ、何を返したか」の列を記録する。複雑なロジックのリファクタリング安全網として使う。

```
ReadDeptStatus("Engineering")         → "active"
WriteApprovalLog("Engineering", 0.85) → (void)
ReadDeptStatus("Marketing")           → "inactive"
```

呼び出し回数だけでは不十分。引数の列と戻り値の両方を含めること。
...
### T-14: 特性テスト（Characterization Test）

*Michael Feathers「レガシーコード改善ガイド」より*

テストのないレガシーコードに対して「現在の挙動を記録する」ことを目的としたテスト。仕様書に合っているかどうかではなく、**今この瞬間のコードが実際にどう動くか**をベースラインとして保存する。

リファクタリング前に特性テストを書いておくことで、変更によって挙動が変わった場合に即座に検出できる。スナップショットテスト（[T-13](#t-13)）はこの考え方の実装形態の1つ。

**適用の流れ**:

1. テスト対象コードを実際に動かして出力を観察する
2. 観察した出力をそのまま期待値として記録する（正しいかどうかは問わない）
3. リファクタリングを実施する
...
### T-15: Object Seam（オブジェクト接合部）via `Implements`
...
## vba-runner analyze ツール

<a id="va-01"></a>

### VA-01: `vba-runner analyze` による静的解析支援

`vba-runner analyze` は VBA ソースを静的解析し、リファクタリング・テスト設計の手がかりとなる情報を出力するツールです。単一ファイルとディレクトリの両方を対象にできます。
...
| `assignmentBlocks`                        | 5件以上連続する代入・Dim 宣言の塊。`shape` によって UDT 化・関数抽出（[R-01](#r-01)）などの手法を選ぶ            | [VA-01b](#va-01b)             |

#### 使用例

```bash
# リファクタリング候補を確認
vba-runner analyze src/vba/ --json | jq '.excelMockTargets'

# モック対象オブジェクトを確認
vba-runner analyze src/vba/ --json | jq '.excelObjectsUsed'
```

#### `--gen-test-dir <dir>`: テスト用ソースの自動生成

```bash
vba-runner analyze src/vba/ --gen-test-dir tests/generated/
```
...
### VA-01b: 連続代入ブロック検出（関数抽出（[R-01](#r-01)）・UDT化候補）
...
出力パラメーターの型によって、ByRef を維持するか戻り値に変えるかの優先度が異なる。

| 型 | 戻り値化の推奨度 | 備考 |
|----|----------------|------|
| プリミティブ（Long, Double 等） | ◎ 推奨 | 出力 1 つ → Function 戻り値、複数 → UDT にまとめる |
| UDT | ○ 設計優先なら推奨 | 戻り値もフィールド全コピーが発生するため ByRef 出力より高コスト。フィールドの数が少なく設計の明快さを優先するなら戻り値化 |
| オブジェクト | ◎ 推奨 | ByVal/ByRef ともポインターコピーのみでコスト差なし。`Set` による差し替えも戻り値で代替できる |
| 配列 | △ 制約あり | ByVal 不可（VBA 仕様）。戻り値化は可能だが制約あり（下記注参照） |

> **注（配列の戻り値化について）**: `Function` の戻り値として配列を返すことは構文上可能だが、以下の制約がある。
> - 呼び出し元は**動的配列変数**（`Dim result() As Long`）でしか受け取れない（固定長配列は不可）
> - `GetArray()(0)` のような戻り値への直接インデックスアクセスは VBA では不可（必ず変数への代入が必要）

なお、現代的なプログラミングの慣習に倣えば ByRef による出力パラメーターは非推奨となるが、ByRef は VBA の文化・慣習であり、無理に排除する必要はない。また、現代の PC 性能では値コピーによるわずかなオーバーヘッドを気にする必要はほとんどなく、過度な最適化はコードのわかりやすさとのトレードオフになりやすい。既存コードと書き方が統一されていることの方が重要です。
...
const runner = new VBARunner('../refactoring/TaskScheduler_Core.bas');
...
```bash
vba-runner analyze src/vba/ --commented-code
```
