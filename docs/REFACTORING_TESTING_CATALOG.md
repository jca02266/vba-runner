...
副作用（Excel 書き込み・ログ・DB 更新など）が処理の途中に散らばっていても、**その副作用の実行タイミングを後ろにまとめても結果が変わらない**場合に適用できる。副作用をすべて末尾（Shell）に移動し、それより前を副作用なしの純粋ロジック（Core）として切り出す。

**Before: 副作用がロジックの途中に散在している**
...
Sub ProcessApproval(dept As String, amount As Long)
    ...
    WriteApprovalLog dept, decision                       ' 副作用①：ログ書き込み（ロジックの途中に混在）

    ' さらにロジックが続く場合もある
    If decision = "TeamLead" Then amount = amount * 0.9

    Sheets("Result").Cells(1, 1).Value = decision         ' 副作用②：結果の書き出し
End Sub
```

**After: 副作用の実行順を変えても問題ないことを確認してから Core と Shell に分離**

```vba
' Core: 純粋関数。副作用なし、入力だけで結果が決まる
Function DecideApproval(dept As String, amount As Long, ByRef finalAmount As Long) As String
    If dept = "Engineering" And amount < 100000 Then
        decision = "TeamLead"
    ElseIf dept = "Sales" Then
        decision = "Manager"
...
Sub ProcessApproval(dept As String, amount As Long)
    ...
    decision = DecideApproval(dept, amount, finalAmount)  ' Core
    WriteApprovalLog dept, decision                       ' 副作用①
    Sheets("Result").Cells(1, 1).Value = decision         ' 副作用②
End Sub
```

Core は [T-01](#t-01) で直接テストでき、Shell は統合テストや手動確認で検証する。

**この例の注意点**:

- `DecideApproval` が `ByRef finalAmount` を受け取る形になっているのは、Core に `amount * 0.9` の計算を含めるために必要だったが、純粋関数として不格好な設計になっている。こうした変形が生じる場合は FC/IS の適用自体を再検討する。
- `WriteApprovalLog dept, decision` の実行タイミングを本当に後ろに移動しても問題ないかは、ログの用途（例: 処理失敗時の記録など）によっては自明でない。実行順の変更が安全かどうか確信が持てない場合は FC/IS を使わず [R-13](#r-13) の Seam で副作用を縫い目として抽出しモックに差し替える手法を選ぶ。

**適用条件（他の手法との違い）**: 副作用の**実行結果が後続のロジック分岐に影響しない**（一方向フロー）場合のみ有効。たとえば DB 書き込みの成否で次の処理が変わる場合は「タイミングを後ろにまとめる」ことができないためこの手法は使えない。その場合は [R-13](#r-13)（Seam）と [T-13](#t-13)（スナップショットテスト）を組み合わせる。

***

<a id="r-13"></a>

### R-13: Seam（縫い目）による副作用の抽出

*Michael Feathers「レガシーコード改善ガイド」より*

**副作用が混在していてテストが書けない状態を、テストが書ける状態に変えるための前段階として行うリファクタリング手法。** 副作用の「塊」を Extract Method で関数として抽出し、テスト時にその関数をモックに差し替えられる差し替えポイント（＝縫い目）を作る。縫い目が作れて初めて、ロジックのリファクタリングとテストが可能になる。

副作用が処理の途中に散在しており、かつ以下の両方の条件が重なる場合に適用する。

* **副作用の出力がロジック分岐に影響する**（[R-12](#r-12) FC/IS のように後ろにまとめられない）

* **副作用への入力値が処理途中の計算結果に依存する**（引数として外に追い出すこともできない）

**Before: 副作用の入出力が処理の途中の計算に絡み合っている**

```vba
' ① Config からステータスを読む → ②の入力に使う route を計算 → ② route に応じた上限を読む → ③ 結果を書く
' 副作用②への入力 route は副作用①の出力から計算されるため、引数として追い出せない
Function ProcessApproval(dept As String, amount As Long) As String
    ' 副作用①: ステータス読み取り
    Dim status As String
    status = Sheets("Config").Cells(GetDeptRow(dept), 2).Value
    If status <> "active" Then
        ProcessApproval = "Rejected": Exit Function
    End If

    ' 中間計算: ①の出力 status を使って承認ルートを決定
    Dim route As String
    route = DecideRoute(dept, status)   ' status は副作用①の結果

    ' 副作用②: route（中間計算の結果）に応じた上限値を別シートから読む
    '          → route の値は呼び出し元では知り得ないため引数に追い出せない
    Dim limit As Long
    limit = Sheets("Limits").Cells(GetRouteRow(route), 2).Value
    ...
    ' 副作用③: 結果を書き込む
    Sheets("Log").Cells(GetNextLogRow(), 1).Value = ProcessApproval
End Function
```

`limit` を引数に追い出しても、呼び出し元で `route` を計算するために `status`（副作用①の結果）が必要になり、問題が上に移るだけ。VBA Runner でテストしようとしても `Sheets` がスタブになり正しく動かない。

**Step 1: 副作用の塊を縫い目として Extract Method する**
...
Function ProcessApproval(dept As String, amount As Long) As String
    ...
    End If
    Dim route As String
    route = DecideRoute(dept, status)          ' 中間計算（純粋ロジック、そのまま）
    Dim limit As Long
...
Function ReadApprovalLimit(route As String) As Long  ' route を引数に取る
...
縫い目②は `route`（中間計算の結果）を引数として受け取る純粋な入出力関数になる。`ProcessApproval` 本体は Sheets に触れなくなり、縫い目をモックに差し替えればロジックだけをテストできる。
...
// 上限超えのケース
runner.mock("ReadDeptLimit", (_dept: string) => 50000);
assert.strictEqual(runner.run("ProcessApproval", ["Engineering", 100000]), "Escalate");
```

縫い目が作れると、`ReadDeptStatus` が `"active"` / `"inactive"` のどちらを返す場合でも、`ReadDeptLimit` がどんな値を返す場合でも、ロジックを自由にテストできる。

**モックの粒度**: 低レベル API（`Sheets`・`MsgBox` など）ではなく**ビジネス的な意味を持つ塊**単位で縫い目を作ること。塊の中にローカル変数への副作用（`ByRef` での変更）が含まれる場合は、モックでもその変数更新を再現する必要がある。

**重要**: 縫い目を作る抽出ステップ自体はテストなしで行うため最も慎重に扱う。抽出後は Excel 上での手動実行で正しく動くことを確認してからスナップショット（[T-13](#t-13)）を記録する。
...
Excel の「マクロの記録」機能が生成するコードは `ActiveSheet`・`ActiveCell`・`Selection` などの暗黙的なレシーバーに依存する。実行中にユーザーが別のシートやセルをクリックするだけで意図しない箇所が操作され、また VBA Runner でのテストも不可能になる。
...
Sub BadMacro()
    Range("A1").Value = 100          ' どのシートの A1 か不明
    ActiveSheet.Name = "Result"      ' 実行時のアクティブシート依存
    Selection.Font.Bold = True       ' 選択状態依存
    Cells(1, 1).Interior.Color = vbYellow  ' シート未指定
End Sub
...
* `ws` を引数にすることで VBA Runner からテスト可能になる（[R-01](#r-01)）
