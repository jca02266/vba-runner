...
**副作用が混在していてテストが書けない状態を、テストが書ける状態に変えるための前段階として行うリファクタリング手法。** 副作用の「塊」を関数抽出（[R-01](#r-01)）で関数として抽出し、テスト時にモックに差し替えられる差し替えポイント（＝縫い目）を作る。
...
縫い目を作る関数抽出（[R-01](#r-01)）自体はテストなしで行うため、抽出後は Excel 上で動作確認してからスナップショット（[T-13](#t-13)）を記録すること。
