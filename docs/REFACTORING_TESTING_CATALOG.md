...
副作用を持たない純粋なロジック（Core）と、副作用を実行するだけの薄い外殻（Shell）を明確に分離する設計パターン。
...
' Shell: Core を呼んで副作用を実行するだけ
Sub ProcessApproval(dept As String, amount As Long)
    Dim decision As String
    decision = DecideApproval(dept, amount)  ' Core を呼ぶ
    WriteApprovalLog dept, decision          ' 副作用はここだけ
End Sub
```

Core は [T-01](#t-01) で直接テストでき、Shell は統合テストや手動確認で検証する。

**適用条件**: 副作用の戻り値が後続のロジックに影響しない（一方向フロー）場合のみ有効。副作用の戻り値が次の条件分岐に入り込む場合は [R-13](#r-13)（Seam + 特性テスト）と [T-13](#t-13)（スナップショットテスト）を組み合わせる。

- [ideas/SIDE_EFFECT_EXTRACTION.md — 適用条件の判断](../ideas/SIDE_EFFECT_EXTRACTION.md#適用条件の判断)
...
テストのために「縫い目（Seam）」を作る手法。副作用が散在する大きな関数の中に、テストのための差し替えポイント（＝縫い目）をExtract Methodで作り出す。

VBA の文脈では、`Sheets` / `Open` / `MsgBox` などが混在する関数から、副作用の「塊」単位で関数を抽出することが縫い目を作ることに相当する。抽出後、その関数をモックに差し替えることでテストが可能になる。

```vba
' Before: 縫い目がない。テスト不可
Function ProcessApproval(dept As String, amount As Long) As String
    Dim status As String
    status = Sheets("Config").Cells(GetDeptRow(dept), 2).Value  ' ← ここが縫い目候補
    If status = "active" Then
        ...
    End If
End Function

' After: 縫い目を作った（副作用を関数として抽出）
Function ReadDeptStatus(dept As String) As String              ' ← 縫い目
    ReadDeptStatus = Sheets("Config").Cells(GetDeptRow(dept), 2).Value
End Function
```

**重要**: 縫い目を作る抽出ステップ自体はテストなしで行うため、最も慎重に扱う必要がある。抽出後は必ず本物のコードを実際に動かして正しく動くことを確認してからスナップショット（[T-13](#t-13)）を記録する。VBA Runner は `Sheets` 等をスタブとして扱うため、この確認は Excel 上での手動実行が必要。

モックの粒度は低レベル API（`Sheets`・`MsgBox` など）ではなく**ビジネス的な意味を持つ塊**単位にすること。塊の中にローカル変数への副作用（`ByRef` での変更）が含まれる場合は、モックでもその変数更新を再現する必要がある。

- [ideas/SIDE_EFFECT_EXTRACTION.md — 詳細](../ideas/SIDE_EFFECT_EXTRACTION.md)
...
- [ ] Code Action（[TODO_NEXT.md](../TODO_NEXT.md)）として `ActiveSheet` → 引数 `ws As Worksheet` への自動リファクタリングを追加（Introduce Parameter）
- [ ] Diagnostics 警告（[TODO_NEXT.md](../TODO_NEXT.md)）: `PasteSpecial` 使用時に「クリップボード依存あり、直接代入への書き換えを検討」を表示
...
- [TEST_FRAMEWORK_GUIDE.md — スナップショットテスト](TEST_FRAMEWORK_GUIDE.md)
- [ideas/SIDE_EFFECT_EXTRACTION.md — スナップショットの内容](../ideas/SIDE_EFFECT_EXTRACTION.md#スナップショットの内容)
