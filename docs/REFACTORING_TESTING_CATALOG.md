...
リファクタリング前に特性テストを書いておくことで、変更によって挙動が変わった場合に即座に検出できる。スナップショットテスト（[T-13](#t-13)）はこの考え方の実装形態の一つ。
...
```bash
npx tsx test-libs/vba-analyzer.ts src/vba/
```
...
```bash
# リファクタリング候補を確認
npx tsx test-libs/vba-analyzer.ts src/vba/ --json | jq '.excelMockTargets'

# モック対象オブジェクトを確認
npx tsx test-libs/vba-analyzer.ts src/vba/ --json | jq '.excelObjectsUsed'
```

#### `--gen-test-dir <dir>`: テスト用ソースの自動生成

```bash
npx tsx test-libs/vba-analyzer.ts src/vba/ --gen-test-dir tests/generated/
```
...
なお、現代的なプログラミングの慣習に倣えば ByRef による出力パラメーターは非推奨となるが、ByRef は VBA の文化・慣習であり、無理に排除する必要はない。また、現代の PC 性能では値コピーによるわずかなオーバーヘッドを気にする必要はほとんどなく、過度な最適化はコードのわかりやすさとのトレードオフになりやすい。既存コードと書き方が統一されていることの方が重要である。
...
```bash
npx tsx test-libs/vba-analyzer.ts src/vba/ --commented-code
```
