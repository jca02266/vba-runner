...
| ドキュメント | 説明 |
|-----------|------|
| **[TUTORIAL.md](TUTORIAL.md)** | vba-extractor での抽出 → VS Code での参照 → AI でのリファクタリング/機能追加 → 書き戻しまでの一連の流れを実例で体験するチュートリアル（コマンド・コードの詳細あり） |
| **[TUTORIAL_AI.md](TUTORIAL_AI.md)** | 同じ流れを、AI への指示文だけで完結させるチュートリアル（コマンドの詳細には触れない） |
...
- **VBARunner API リファレンス**（`run` / `eval` / `set` / `setConstants` / `mockDate` / `registerComObject`）、**assert ユーティリティ**（`strictEqual` / `deepStrictEqual` / `ok` / `isTrue` / `isFalse` / `fail`）
