...
MS-VBAL の §3.3.2 では整数トークンを
`integer-literal ["%" / "&" / "^"]` と定義しており、`^` は LongLong の
数値リテラルサフィックスである。一方、§3.3.1 では `^` を特殊トークンとしても
定義している。したがって、数値と `^` の間に空白があるかどうかで、同じ文字列が
サフィックス付きリテラルか冪乗演算子かに分かれる。

```
入力: 10^-2
字句解析: Number("10^")  OperatorMinus("-")  Number("2")
結果: LongLong の 10 から 2 を減算して 8
入力: 10 ^ -2
字句解析: Number("10")  OperatorPower("^")  OperatorMinus("-")  Number("2")
結果: 10 の -2 乗で 0.01
入力: 2^2
字句解析: Number("2^")  Number("2")   ← ^ が吸収されるため、通常は構文エラー
入力: 2 ^ 2
字句解析: Number("2")  OperatorPower("^")  Number("2")  ← 4
```

| 書き方 | `^` の分類 | 結果の例 |
|--------|------------|----------|
| `10^-2` / `10^ -2` / `10^ - 2` | `10^` は LongLong サフィックス | `8` |
| `10 ^ -2` / `10 ^ - 2` | `^` は冪乗演算子 | `0.01` |
| `2^2` | `2^` を Number として吸収 | 構文エラーになり得る |
| `2 ^ 2` | `^` を OperatorPower として切り出し | `4` |
| `x^2` | 識別子の後は数値サフィックス文脈外 | 冪乗演算 |

`^` の後ろの空白は、数値と `^` が隣接している場合のサフィックス分類を変えない。
今回の Excel 実機マトリックスでは、次の出力を確認した。

```text
10^-2=8 10^ -2=8 10^ - 2=8 10 ^ -2=0.01 10 ^ - 2=0.01
```

この空白境界は MS-VBAL のトークン定義から導かれる仕様上の区別であり、
`^` の後ろの空白を読み飛ばしてから冪乗かどうかを判定してはならない。
