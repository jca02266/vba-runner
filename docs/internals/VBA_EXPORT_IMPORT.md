...
`vbaProject.bin` はVBAソースをそのまま格納した単一バイナリではない。OOXMLブック内の
`xl/vbaProject.bin` パーツであり、その中身は [MS-CFB] のCompound File Binary (CFB)
コンテナである。CFBのルート配下にVBAプロジェクト用のストレージとストリームがあり、
各モジュールのソース、プロジェクトメタデータ、コンパイルキャッシュを別々に保持する。

したがって、処理単位は次の三層に分けて考える。

1. **OOXML層** — ZIPパーツ、Content Types、Workbookリレーション。
2. **CFB層** — Directory Entry、ストレージ、FAT/miniFAT、ストリームのバイト列。
3. **OVBA層** — `dir` のプロジェクト／モジュールレコード、VBAソースの圧縮表現、
   `PROJECT`/`PROJECTwm` の名前情報。

CFBのストリーム名と、`dir` に記録されたモジュール名・ストリーム名は別の情報である。
モジュールを追加・改名・削除するときは、CFBストリームだけでなく `dir` と
`PROJECTwm` も同時に更新しなければならない。逆に、`PROJECT` はテキスト形式のため、
バイナリストリームとして扱ったり、VBAソースの圧縮データとして解釈したりしてはいけない。

`vba-extractor` が実際に読み書きする主要な単位は次のとおりである。

| 単位 | 内容 | 更新時の注意 |
|------|------|--------------|
| `VBA/dir` | OVBA圧縮されたプロジェクト／モジュール定義 | モジュール順、型、`MODULEOFFSET`を整合させる |
| `VBA/<stream>` | モジュールごとの圧縮VBAソース | `MODULEOFFSET`以降がソース。source-onlyでは0から開始 |
| `VBA/_VBA_PROJECT` | コンパイル済みプロジェクトキャッシュ | source-onlyでは無効化ヘッダーに置換 |
| `VBA/__SRP_*` | パフォーマンスキャッシュ | source-onlyでは削除 |
| `/PROJECT` | プロジェクト名・参照・`Document=`等のテキスト | コードページでエンコード |
| `/PROJECTwm` | モジュール名のUnicode対応表 | `PROJECT`/`dir`のモジュール集合と同期 |

### VBAソースの圧縮と復元

モジュールストリームのソース部分は、通常のUTF-8テキストではなく [MS-OVBA] の
Chunk圧縮形式で保存される。圧縮データは複数のチャンクからなり、各チャンクは圧縮済み
または非圧縮のいずれかである。`export` は `MODULEOFFSET` を使ってプリアンブルを
飛ばし、チャンクを展開して指定コードページのテキストへ戻す。`import` はソースを
同じChunk形式へ圧縮し、`dir` のオフセットを更新する。

CFBのセクター境界やFATの割り当ては、VBAの論理モジュール形式とは別の責務である。
ストリームのサイズ変更時にCFBライブラリがFATとminiFATを再配置するため、アプリケーション
コードがセクター番号を直接計算してはいけない。

## バイト単位の読み方

ここでは、`vbaProject.bin` 内の論理データをバイト列として読むときの規則を示す。
CFBコンテナそのもののセクター番号・FATチェーンはCFBライブラリが解決するため、
以下はストリームを取得した後の内容に対する説明である。

### 共通の整数表現

OVBAの整数フィールドはリトルエンディアンで格納される。たとえば、4バイト列
`78 56 34 12` は `0x12345678` と読む。
...
| `u16` | 2 | 下位バイト、上位バイトの順 |
| `u32` | 4 | 下位バイトから順に連結 |
| GUID | 16 | GUIDの各フィールドは仕様で定められた混在エンディアン |

`dir` のレコード長や`MODULEOFFSET`をビッグエンディアンで読むと、後続レコードの
位置がずれ、モジュール一覧全体を誤って解釈する。文字列のコードページは数値の
エンディアンとは別の問題であり、`PROJECT`と`dir`のANSI名はプロジェクトの
`PROJECTCODEPAGE`に従ってデコードする。

### `dir` レコードのバイト配置

展開後の`dir`は、概ね次の繰り返しで構成される（レコード固有の拡張部分を除く）。

```
offset +0  : recordId  u16  (2 bytes, little-endian)
offset +2  : size      u32  (4 bytes, little-endian)
offset +6  : data      size bytes
```

たとえば、`19 00 04 00 00 00 41 00 00 00` は、`recordId=0x0019`、
`size=4`、データ4バイトを表す。次のレコードは常に`offset + 6 + size`から始まる。
`PROJECTVERSION`など一部のレコードは、規定されたデータの後に追加の固定長フィールドを
持つため、`size`だけで意味を推測せず、[MS-OVBA]のレコード定義に従って読む。

### モジュールストリームのオフセット

`MODULEOFFSET`は、モジュールストリームの先頭から圧縮ソースが始まる位置を示す
4バイトの符号なし値である。値が`00 00 00 00`なら、ストリームのバイト0から
圧縮チャンクを読む。値が`10 00 00 00`なら、バイト0〜15はプリアンブルであり、
バイト16からソースを読む。

source-only形式では、プリアンブルを除去して`MODULEOFFSET=0`にする。オフセットを
変更せずにソースだけを差し替えると、展開器はP-Codeまたは未整合なバイト列をソースと
して読み込むため、文字化けやVBEの読み込み失敗につながる。

### 圧縮チャンクの先頭バイト

モジュールソースは、ストリーム全体を一つのテキストとして扱わず、チャンク単位で展開する。
各チャンクはヘッダーを含み、ヘッダーのサイズビットと圧縮フラグからチャンク長と
圧縮／非圧縮を判定する。チャンクの境界を越えてデコードしたり、チャンクヘッダーを
ソース文字として扱ったりしてはいけない。

`vba-extractor` では圧縮・展開をOVBA圧縮ヘルパーへ委譲し、モジュールの文字コード変換は
その前後で行う。したがって、次の順序を維持する。

```
CFB stream bytes
  → MODULEOFFSET を適用
  → OVBA chunk decompress
  → PROJECTCODEPAGE でテキスト化
```

import時は逆順に、ソースをコードページでバイト化してからOVBAチャンクへ圧縮し、
新しい圧縮バイト列の長さに合わせて`MODULEOFFSET`とCFBストリームを更新する。

### `PROJECTwm` のバイト列

`PROJECTwm`は、モジュール名のUnicode対応を保持するバイナリストリームである。
これはVBAソースの圧縮データではなく、モジュール名をANSI名からUnicode名へ対応付ける
ためのテーブルとして扱う。モジュールを追加・改名・削除した場合は、`PROJECT`の
`Module=`／`Document=`行、`dir`のモジュールブロック、CFBストリーム名と同じ集合に
なるよう更新する。バイト列をテキストとして改行分割したり、UTF-8へ変換して保存したり
すると、Unicode名の対応が壊れる。

### バイト検査時の境界条件

- CFBストリームの末尾を越えて`Get`しない。短いストリームは不足分をゼロ埋めして
  解釈せず、破損として扱う。
- `dir`レコードの`size`がストリーム末尾を越える場合は、後続レコードを推測しない。
- `MODULEOFFSET`がストリーム長より大きい場合は、ソースなしの破損モジュールとして扱う。
- 圧縮チャンクの終端がストリーム末尾と一致しない場合は、展開成功とみなさない。
