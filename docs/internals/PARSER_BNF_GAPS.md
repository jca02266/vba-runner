...
| §5.4.2.2 While | ✅ | — |
| §5.4.2.3 For | ⚠️ | `Next i, j`（複数変数）未対応 → `Next j, i` でパースエラー |
| §5.4.2.4 For Each | ⚠️ | §5.4.2.3 と同根。`Next` 後の複数変数未対応 |
| §5.4.2.5 Exit For | ✅ | — |
| §5.4.2.6 Do | ✅ | — |
| §5.4.2.7 Exit Do | ✅ | — |
...
| §5.4.3.2 Const | ❌ | `Const A = 1, B = 2`（複数定数の同時宣言）→ パースエラー「unexpected ','」 |
| §5.4.3.3 ReDim | ❌ | `ReDim arr1(5), arr2(10)`（複数変数）→ パースエラー |
| §5.4.3.3 ReDim | ❌ | `ReDim obj.Arr(5)`（メンバーアクセス対象）→ パースエラー |
| §5.4.3.3 ReDim | ❌ | `ReDim .Items(n)`（With式対象）→ パースエラー |
| §5.4.3.4 Erase | ❌ | `Erase arr1, arr2`（複数要素）→ パースエラー |
...
| §5.4.4.1 Error | ✅ | — |
| §5.4.4.2 Resume | ✅ | — |
| §5.4.4.3 On Error | ❌ | `On Error GoTo -1` → `-` と `1` が別トークンになりパースエラー（evaluator 側は `-1` を処理できる実装あり） |
...
| 節 | 状態 | 問題・VBA例 |
|---|---|---|
| §5.4.5.1 Open | ⚠️ | `Open "f.txt" As #1`（`For mode` 省略）→ `For` を必須消費するためパースエラー |
...
| §5.4.5.1.1 File Numbers | ⚠️ | `Seek 1, 100` / `Print 1, "x"` 等（`#` なし unmarked-file-number）→ 多数のパーサーで `#` を必須化しており拒否する（Seek/LineInput/Width/Print/Write/Input/Put/Get の全8ステートメント共通） |
| §5.4.5.2 Close/Reset | ✅ | — |
| §5.4.5.3 Seek | ⚠️ | `#` 必須化（§5.4.5.1.1 と同根） |
| §5.4.5.4 Lock | ⚠️ | `Lock #1, To 100`（start 省略の `To end` 形式）→ `start = parseExpression()` を先読みするため未対応 |
| §5.4.5.5 Unlock | ⚠️ | Lock と同根 |
| §5.4.5.6 Line Input | ⚠️ | `#` 必須化 + `variable-name` が Identifier のみ（`Line Input #1, arr(i)` 不可） |
| §5.4.5.7 Width | ⚠️ | `#` 必須化 |
| §5.4.5.8 Print | ⚠️ | `#` 必須化 |
| §5.4.5.9 Write | ⚠️ | `#` 必須化。output-list 内の Spc/Tab が未処理（式として評価される可能性あり） |
| §5.4.5.10 Input | ⚠️ | `#` 必須化 |
| §5.4.5.11 Put | ⚠️ | `#` 必須化 |
| §5.4.5.12 Get | ⚠️ | `#` 必須化 |
...
| 問題 | 場所 | VBA例 |
|---|---|---|
| `On Error GoTo -1` パースエラー | §5.4.4.3 | `On Error GoTo -1` |
| `ReDim` 複数変数未対応 | §5.4.3.3 | `ReDim arr1(5), arr2(10)` |
| `Erase` 複数要素未対応 | §5.4.3.4 | `Erase arr1, arr2` |
| `#` なし file-number 未対応（8ステートメント共通） | §5.4.5.1.1 | `Seek 1, 100` |

### 🟠 中（実用コードで踏む可能性あり）

| 問題 | 場所 | VBA例 |
|---|---|---|
| `Const A = 1, B = 2` 複数定数 | §5.4.3.2 | `Const MAX = 100, MIN = 0` |
| `ReDim obj.Arr(5)` メンバーアクセス | §5.4.3.3 | `ReDim Me.Data(n)` |
| `ReDim .Items(n)` With式 | §5.4.3.3 | With ブロック内 ReDim |
| `Next i, j` 複数変数 | §5.4.2.3 | ネストした For の1行 Next |
| `Open "f.txt" As #1`（For 省略） | §5.4.5.1 | モード省略 Open |
...
| 節 | 状態 | 問題・VBA例 |
|---|---|---|
| §3.4.1 #Const | ❌ | `#Const DEBUG = True` → レキサーは `#` を `OperatorHash` として出力、パーサーは行頭の `#` 文を一切処理しないのでパースエラー |
...
| 節 | 状態 | 問題・VBA例 |
|---|---|---|
| §3.4.2 #If | ❌ | `#If DEBUG Then ... #End If` → §3.4.1 と同根。`#If`/`#ElseIf`/`#Else`/`#End If` いずれも未対応 |
...
| 節 | 状態 | 問題・VBA例 |
|---|---|---|
| §5.2.3.1 モジュール変数宣言 | ⚠️ | `Global x As Integer`（`Global` が未キーワード化 → Identifier 扱い → パースエラー）。`Dim Shared` は既報（バッチ1） |
| §5.2.3.1.1 変数宣言 | ✅ | — |
| §5.2.3.1.2 WithEvents | ✅ | — |
| §5.2.3.1.3 配列次元と境界 | ✅ | — |
| §5.2.3.1.4 変数型宣言 | ✅ | — |
| §5.2.3.2 Const 宣言 | ❌ | `Const A = 1, B = 2`（複数）→ 既報（バッチ1 §5.4.3.2）。同一の `parseConstDeclaration` |
| §5.2.3.3 UDT 宣言 (Type) | ⚠️ | メンバー名として `End` が使用不可。ループ終端が `KeywordEnd` チェックのため `End As Long` で早期終了 |
| §5.2.3.4 Enum 宣言 | ⚠️ | `Global Enum MyEnum` → `Global` が未キーワード化のため `Enum` ディスパッチに到達せずパースエラー |
...
| 問題 | 場所 | VBA例 |
|---|---|---|
| 条件付きコンパイル `#Const`/`#If` 完全未対応 | §3.4.1/2 | `#If Win64 Then` / `#Const DEBUG = True` |

### 🟠 中（実用コードで踏む可能性あり）

| 問題 | 場所 | VBA例 |
|---|---|---|
| `Global` キーワード未定義（変数・Const・Enum に影響） | §5.2.3.1/2/4 | `Global Const MAX = 100` |
...
| `Global Enum` 未対応 | §5.2.3.4 | `Global Enum Color` |
...
| §5.3.1.1 Procedure Scope | ⚠️ | `Global Sub Foo()` → `Global` が Identifier 扱いのため scope='public' に昇格しない（既報バッチ2） |
...
| `Global Sub Foo()` スコープ未設定 | §5.3.1.1 | バッチ2 `Global` 既報と同根 |
...
| 節 | 状態 | 問題・VBA例 |
|---|---|---|
| §5.6.7 TypeOf…Is | ⚠️ | `TypeOf x Is ADODB.Recordset`（ドット修飾型名）→ `parseTypeOf` が1トークンしか読まず `.Recordset` がパースエラー（Implements と同根） |
...
| 問題 | 場所 | VBA例 |
|---|---|---|
| `TypeOf x Is ADODB.Recordset` ドット修飾型名未対応 | §5.6.7 | `If TypeOf conn Is ADODB.Connection Then` |
...
| §5.6.16.2 cc-expression | ❌ | 条件付きコンパイル式。§3.4 と同根（バッチ2既報） |
