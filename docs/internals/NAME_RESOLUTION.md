...
事前検査の順序は次の三段階で固定する。まず手続き全体からローカル変数・引数・
配列・型を収集し、クラスフィールドや既知の環境名と合わせて識別子の定義集合を
作る。次に使用箇所ごとに、値、手続き、既知の名前、未定義のいずれかへ束縛する。
未定義の場合だけ未定義識別子の診断を出し、値に対しては配列／既定メンバーなど
使用側の要求と型を比較する。手続きに対する引数個数・ByRef検査は、手続きへの
束縛が確定した後にだけ実行する。したがって `name(arg)` という構文だけで関数と
仮定せず、値の既定メンバー呼び出しと手続き呼び出しを同じ名前解決結果から分岐
させる。この順序の回帰はエンジンの
[`call-arity-matrix.test.ts`](../../tests/spec/call-arity-matrix.test.ts)で検証する。

この契約が導入される前は、実行時の`Me.__classDef__`検索、事前検査の
`Environment`検索、LSPのシンボル表が別々にクラスPrivateメンバーを扱っていた。
そのため、クラス内のイベント処理から同じクラスのPrivate手続きを呼ぶコードは
実行できるのに、LSPだけが「未定義」と報告する不一致が発生した。現在は実行時と
事前検査の手続き解決を統合済みで、同種の回帰テストをエンジン側に持つ。LSPの補完・定義検索など表示目的の
シンボル表は実行Resolverそのものではないが、クラススコープを保持し、Private
メンバーをワークスペースのモジュール名として公開しない契約に統一している。
