...
### 評価とバグの記録単位（フォーマット v2）

評価記録は一回の探索評価、バグ記録はその評価で確認された不具合の追跡記録である。
この二つを一つの文書へ混在させない。

- 新規EVには`evaluationRecordVersion: 2`を付ける。
- 一つのEVは一つの具体的な挙動、入力条件、期待値、判定だけを扱う。
- EV本文には`## 評価内容`、`## 期待値`、`## 結果`、`## 判定`を置く。
...
依頼する。レビューでは、EVが一つの挙動に限定されているか、期待値・結果・判定が具体的か、
一時実行情報を証拠として誤用していないか、BUGの横展開・真因分析・回帰テストの状態が
整合しているかを確認する。指摘が残る間は完了状態へ遷移しない。
...
状態が完了かどうかはEVのスナップショットの`status`で判断できるため、新しい状態更新では
`completedAt`を生成しない。EVがその状態になった時刻は`events[].occurredAt`に記録し、
これを評価単位の時系列集計の正本とする。`completedAt`は過去のスナップショットとの
互換用に読み取るだけであり、過去記録の評価日しかない場合も旧記録由来の日時として表示する。
CLIが作成する新しい結果スナップショットには`stateVersion: 1`を付け、対応する
`events`がない状態を`validate`で拒否する。旧形式のスナップショットはこの検査の対象外
であり、移行時にイベントを追加する。

`eval audit`は、旧形式の評価日を持つレガシー記録を除き、すべてのEVがイベント履歴または
結果スナップショットのいずれかに現れることも検査する。どちらにも現れないEVは時系列
グラフから欠落して状態別集計と一致しなくなるため、IDと最終状態を示す整合性エラーに
する。監査は不足する日時や状態を推測して補完せず、Git履歴などの根拠を確認したうえで
履歴移行としてイベントを追加する。

## 現行CLIと役割

待機後の収束チェックポイントは、キャンペーンに依存しない
`evaluation/checkpoints/EVALUATION_LOOP.yml`へ監査結果・判断・判断結果
（既存候補の`claim`または新規候補の`create-candidate`）・次候補を追記する。
...
`*.result.yml` は候補に紐付く最新EVの状態スナップショットであり、過去のEV状態を上書きする。
そのため、実機照合待ちなどを後から解消した場合にも評価単位の時系列を失わないよう、
EVの完了または再判定のたびに同じ候補の `*.events.yml` へ評価イベントを追記する。
イベントには`candidateId`、`evaluationId`、`status`、`fromStatus`（2回目以降）、
`occurredAt`を保存する。同じ候補で同じ状態のEVが続いても、異なるEVであれば許容する。
最後のイベントはresultの最新状態・評価IDと一致しなければならない。

既存EVに履歴がないことは許容する。履歴をGit履歴から補完する場合は、EVごとに評価状態を
復元し、既存イベントとの重複を除いて追記する。旧履歴の候補にresultスナップショットが
存在しなくても、イベント履歴自体は検証・レポート対象にできる。`scripts/eval-report.mjs`
はイベントファイルを読み取り、EV単位の状態遷移履歴として表示する。根拠のない日時や状態は
推測して追加しない。移行後に対応する修正・検証コミットを特定できた旧イベントだけは、
`node scripts/backfill-evaluation-events.mjs --refine --apply`で`sourceCommit`とコミット時刻を
補完できる。対応付けが曖昧なイベントは移行時刻を維持し、推測で細分化しない。
