...
- [ ] **読み方が複数ある** — Main Sub を読むだけで全体構造がわかるか？
- [ ] **複雑度が分散** — 各関数が 20 行程度か？
- [ ] **責任が明確** — 各関数の目的が関数名からわかるか？
- [ ] **パラメーターが明確** — 「何が入力で、何が出力か」が関数シグネチャからわかるか？
...
| 他の用途でも使われる | 名前付き範囲はマクロ以外（数式・印刷範囲など）でも参照されることがあり、リファクタリング時に影響範囲が見えにくい |
| ドキュメントを残せない | 名前付き範囲には説明文を付ける場所がほぼない。設定シート方式であれば設定値の隣のセルに自由にコメントや単位を記載できる |
