...
VBA Runner には `src/engine/mock/MockExcel.ts` として `MockApplication` と `MockWorksheet` が実装済みです。
...
1. **Sandboxルートの指定**: MemoryFileSystemではデフォルトの仮想ルートは
   `/sandbox`です。これはリポジトリ上のディレクトリではありません。
   `sandboxRoot`を指定した場合は、そのルート名が仮想パスの基点になります。
...
`vba-runner run`もVBA実行時はMemoryFileSystemを使用するため、VBAが作成した
ファイルはCLI終了後にホストへ残りません。CLIが入力ソースをNode.jsの`fs`で
読み込むことは、VBA実行時のファイルI/Oとは別の処理です。実ファイルをVBAから
扱う場合は、`NodeFileSystem`と明示的な`sandboxRoot`を注入してください。
...
const fs = (vbaRunner.evaluator as any).fs;  // fs は private のため型キャストが必要
