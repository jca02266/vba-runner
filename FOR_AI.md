...
1. **VBAファイルの全文読み禁止** — `cat` / `Read` でファイル全体を読むことは禁止。
   常に `vba-runner analyze` の行番号を使って必要な範囲だけ読む。
...
## VBAの落とし穴（AIが間違えやすいポイント）

- 単一行If文（`If 条件 Then 文 [Else 文]`）に`ElseIf`は使えない。`ElseIf`はブロックIf（`If...Then` / `ElseIf` / `End If`）専用。複数条件を1行で書きたい場合は単一行Ifを複数行に分けて書く。
- `Application.OnTime`に1秒未満のオフセットを渡すコード（`Now + 0.4/86400`等）は実機で無限ループに陥る場合がある: `Now()`は秒未満を切り捨てるため、計算した目標時刻が既に過去になることがあり、`OnTime`は過去の時刻だと即座に発火する。タイマー処理が自己再スケジュールする実装だと、これが延々と繰り返されてUIスレッドを埋め尽くす（CPU使用率は低いまま「応答なし」になるため気づきにくい）。オフセットは必ず1秒以上にすること。これは実VBA自体の仕様上の罠であり、vba-runnerの`excelStub`は`OnTime`/`OnKey`を未実装のためテストでも検出できない。
...
vba-runner analyze <対象ディレクトリ>

# Step 1-c: コメントアウトコード確認
# コメントアウトコードが多いソースの場合無駄なトークン削減のためこの出力を使って
# 効果的に削除候補を判断、削除する
vba-runner analyze <対象ディレクトリ> --commented-code
```
...
vba-runner analyze <対象パス> --json > .vba-baseline.json

# Phase 6 で差分を表示（改善 ✅ / 悪化 ⚠ を自動判定）
vba-runner analyze <対象パス> --diff .vba-baseline.json
```
...
### vba-runner analyze（静的解析）

```bash
vba-runner analyze <パス>                          # テキスト出力
vba-runner analyze <パス> --json                   # JSON出力（プログラム連携）
vba-runner analyze <パス> --json > baseline.json   # ベースライン保存
vba-runner analyze <パス> --diff baseline.json     # 差分表示（効果測定）
vba-runner analyze <パス> --diff baseline.json --json  # 差分をJSON出力
vba-runner analyze <パス> --outline                # AI向けコンパクト要約
vba-runner analyze <パス> --summary-only           # ワークスペース集計のみ
vba-runner analyze <パス> --commented-code         # コメントアウトコード候補のみ
```
...
### vba-runner format（フォーマッター）

**フォーマッターはAIのワークフローには含めない。リファクタリング後に人間が任意で実行するツール。**

```bash
vba-runner format <ファイルまたはディレクトリ>            # stdout に出力
vba-runner format <ファイルまたはディレクトリ> --check    # 差分チェック（exit 1）
vba-runner format <ファイルまたはディレクトリ> --write    # 上書き
vba-runner format <ファイルまたはディレクトリ> --indent-size=2
```
