...
node test-libs/vba-analyzer.cjs <対象パス> --json > .vba-baseline.json

# Phase 6 で差分を表示（改善 ✅ / 悪化 ⚠ を自動判定）
node test-libs/vba-analyzer.cjs <対象パス> --diff .vba-baseline.json
```
...
node test-libs/vba-analyzer.cjs <パス> --json > baseline.json   # ベースライン保存
node test-libs/vba-analyzer.cjs <パス> --diff baseline.json     # 差分表示（効果測定）
node test-libs/vba-analyzer.cjs <パス> --diff baseline.json --json  # 差分をJSON出力
