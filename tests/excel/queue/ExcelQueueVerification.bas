...
Private Sub VerifyPowerOperatorBoundary()
    ...
    Err.Clear
    EmitResult "XL-231 2^3%=" & CStr(2 ^ 3%) & " 2^3&=" & CStr(2 ^ 3&) & _
        " 2^3!=" & CStr(2 ^ 3!) & " 2^3#=" & CStr(2 ^ 3#) & _
        " 2^3@=" & CStr(2 ^ 3@) & " ERR=" & CStr(Err.Number)
    Err.Clear
    On Error GoTo 0
End Sub
...
Private Sub VerifyDateSerialNormalizationBoundary()
...
Private Sub VerifyFinancialIntermediateFollowup()
    ...
    huge = 1E+308
    On Error Resume Next
    Err.Clear: value = FV(0.5, 2, -huge * 0.48, huge * 0.8, 0): errNo = Err.Number
    EmitResult "XL-214 FV-CANCELLATION ERR=" & CStr(errNo)
    Err.Clear: value = PV(0.5, 2, huge * 0.8, -huge * 1.7, 0): errNo = Err.Number
    EmitResult "XL-215 PV-CANCELLATION ERR=" & CStr(errNo)
    Err.Clear: value = PMT(0.5, 2, huge * 0.8, -huge * 1.7, 0): errNo = Err.Number
...
Private Sub VerifyDictionaryExistsRequired()
    Dim dict As Object, value As Variant, errNo As Long
    Set dict = CreateObject("Scripting.Dictionary")
    dict.Add "key", True
    On Error Resume Next
    Err.Clear: value = dict.Exists: errNo = Err.Number
    EmitResult "XL-202 OMITTED ERR=" & CStr(errNo) & " TYPE=" & TypeName(value)
    Err.Clear: value = dict.Exists(): errNo = Err.Number
...
Private Sub VerifyRadixCurrencyDecimalSignMatrix()
    ...
    EmitValueAndType "XL-095 CCur-MINUS-HEX", value, errNo
    Err.Clear: value = CCur("+&H10"): errNo = Err.Number
    EmitValueAndType "XL-096 CCur-PLUS-HEX", value, errNo
    Err.Clear: value = CDec("+1"): errNo = Err.Number
    EmitValueAndType "XL-097 CDec-PLUS-DEC", value, errNo
    Err.Clear: value = CDec("-1"): errNo = Err.Number
    EmitValueAndType "XL-098 CDec-MINUS-DEC", value, errNo
    On Error GoTo 0
End Sub

Private Sub VerifyRadixExplicitSignTargetMatrix()
    Dim value As Variant, errNo As Long
    On Error Resume Next
    Err.Clear: value = CDec("+&H10"): errNo = Err.Number
    EmitValueAndType "XL-091 CDec-PLUS-HEX", value, errNo
    Err.Clear: value = CCur("+&O10"): errNo = Err.Number
    EmitValueAndType "XL-092 CCur-PLUS-OCT", value, errNo
    Err.Clear: value = Val("+&O10"): errNo = Err.Number
    EmitValueAndType "XL-093 Val-PLUS-OCT", value, errNo
    Err.Clear: value = CDec("-&O10"): errNo = Err.Number
    EmitValueAndType "XL-094 CDec-MINUS-OCT", value, errNo
    On Error GoTo 0
End Sub
