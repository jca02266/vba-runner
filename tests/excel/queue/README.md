...
`VerifyPowerOperatorBoundary`の結果ラベルは、ソースに記述した式の空白配置を
示す。VBEでブックを保存すると、VBEの自動整形によって演算子の空白や単項`+`
が変更されることがあるため、保存後のモジュールを再抽出しても元の記述を
完全には復元できない。VBEが保持できない空白配置やコンパイルエラー境界は
`tests/vba/CompileError.bas`とvba-runnerの構文テストで検証し、Excel結果の
ラベルだけから保存前の字句配置を推測しない。
...
- XL-269: 修飾UDT配列とFunction戻り値のTypeName・VarType表記境界
