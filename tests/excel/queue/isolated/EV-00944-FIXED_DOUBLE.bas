Attribute VB_Name = "P944FixedDouble"
Option Explicit

Public Function RunCompileProbe() As String
    Dim flows(0 To 2) As Double, value As Variant, errNo As Long
    flows(0) = -100: flows(1) = 50: flows(2) = 100
    On Error Resume Next
    Err.Clear: value = Empty
    value = MIRR(flows, 0.1, 0.12)
    errNo = Err.Number
    RunCompileProbe = "XL-246 CASE=FIXED_DOUBLE COMPILE=0 ERR=" & CStr(errNo) & _
