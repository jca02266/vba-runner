...
- [x] **インメモリ・ファイルシステム (In-Memory FS)**
  ...
  - [ ] **VFS と Node.js FS の相互互換性向上**: VFS モードで `./workspace` や `../foo/bar/workspace` のような相対パスを受け入れても、内部的には `/workspace` を起点にルートディレクトリとして解決する仕組みの実装。これにより、同じテストコード（相対パスを使用）が VFS と実ファイルシステムの両モードで同一に動作するようになる。
