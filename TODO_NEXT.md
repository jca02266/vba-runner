...
- [ ] **Code Actions（右クリック → Refactor メニュー）**
  ...
  | アクション | トリガー | 難度 |
  |---|---|---|
  | **変数の導入**（Introduce Variable） | 式を選択 → 直前行に `Dim name = <選択式>` を挿入し選択箇所を `name` に置換 | ★☆☆ |
...
- [ ] **コールグラフの可視化**
  プロシージャ間の呼び出し関係を VS Code の Webview パネルにグラフ表示する。
  レガシー VBA の「どの関数がどこから呼ばれているか」の把握を視覚的に支援する。
