...
- **現象**: `Public Function Close() As Boolean` のようにキーワード名の関数を宣言し、その中で `Close = value` と書くと、パーサーが `Close` を file I/O キーワードとして解釈してエラーになる。`Open = value` も同様。
- **修正**: `parseStatementInner` の `KeywordOpen` / `KeywordClose` 分岐に `peek(1) !== OperatorEquals` ガードを追加し、識別子ブランチに `KeywordOpen` / `KeywordClose` を追加。`COMPAT_KW_EXPR` に `KeywordOpen` / `KeywordClose` を追加して `parsePrimary()` でも識別子として扱われるようにした。
- | `contextual-keyword-as-identifier.test.ts` (Section 6: Open/Close を Function 名・戻り値代入に使える (B-8))
