...
### BNF vs 再帰下降パーサー 乖離リスト（2026-06-13 調査）
...
### 組み込み関数の括弧無し呼び出し（auto-call）のメタデータ化

- ❌ **組み込み関数の「必須引数0個なら括弧無しで自動呼び出し」判定が個々の関数登録に依存している**
  - 現状: ユーザー定義の Sub/Function は、パース時に得られる `parameters` 配列（`isOptional`/`isParamArray`）から `evaluateExpression` の `Identifier` ケースが**汎用的に**「必須引数0個 → 自動呼び出し」を判定する（`evaluator.ts` の `requiredCount === 0` 判定）。
  - 一方、組み込み関数（`Rnd`, `Now`, `Date`, `Time`, `Timer`, `Error`, `DoEvents`, `FreeFile`, `GetObject` 等）は素の JS 関数として `env.set()` で登録されており、VBA 側の「引数が Optional か」というメタ情報を持たない。そのため `(fn as any).__vbaAutoCall__ = true` という個別フラグを登録時に手動で付与する代替手段が使われている。
  - 問題点: このフラグは関数を新規登録するたびに**人間が忘れずに付ける**ことに依存しており、実際に `Rnd`（括弧無しで `Type mismatch` になるバグとして発覚）・`DoEvents`・`FreeFile`・`GetObject` がフラグ漏れだった（2026-06-21 修正）。
  - JS の `Function.prototype.length` から自動推論できないか検討したが不採用:
    1. TypeScript の `val?: any` は JS にコンパイルすると通常の必須パラメータになり（デフォルト値ではない）、`.length` は減らない。
    2. `InStr` 等は `(...args: any[]) => {}` という可変長引数で実装されており、`.length` は常に 0 になるため、本来必須引数があるのに誤って自動呼び出ししてしまう。
  - **今後の対応案**: 組み込み関数の登録を「JS 関数 + 必須引数の数（またはユーザー定義同様の Optional 情報）」を必ずセットで渡す仕組みに置き換え、`__vbaAutoCall__` のような boolean フラグの個別管理を廃止する。例えば `registerBuiltin(name, fn, requiredArgCount)` のような一元化されたヘルパーを経由させ、未指定時はエラーにする（サイレントな登録漏れを防ぐ）など。約100個ある組み込み関数登録箇所すべてに影響するため、着手時は影響範囲の洗い出しから行う。
