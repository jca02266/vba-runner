...
- ⚠️ **組み込み関数に VBA パラメーターメタデータ（`BuiltinParamSpec[]`）を持たせる仕組みを導入（実装中、段階移行）**
  ...
  - 実装: `registerBuiltin(name, fn, params: BuiltinParamSpec[], variants?)` — `__vbaParamSpec__` を関数に付与し、`__vbaAutoCall__` を必須引数0個から自動算出する（手動フラグは廃止）。呼び出し時は `resolveCallArgs`（`evaluator.ts`、グローバル組み込み関数呼び出し・`VBA.Func(...)` 修飾呼び出し・`obj.Method(...)` メソッド呼び出し・式の結果を呼ぶ汎用フォールバックの4箇所で共有）が `__vbaParamSpec__` の有無を見て、検証・名前解決を行うか今までの単純な位置引数評価のみで済ませるかを切り替える（未移行の関数は無変化）。
  ...
  - 移行済み: `Now`/`Date`/`Time`/`Timer`/`Rnd`/`FreeFile`/`DoEvents`/`Error`/`GetObject`（Batch 1）、`InStr`/`InStrB`（Batch 3 先行）。テスト: `builtin-arg-metadata.test.ts`。
  - 残作業: 約190個ある残りの組み込み関数登録箇所の段階移行（`Mid`/`Left`/`Right`/`Format`/`MsgBox` 等の Optional 引数を持つ関数 → 単純な1必須引数の関数 → 残り）。`MockWorksheet.Range`（1引数/2引数で意味が変わる、かつ2引数版は未実装）等のモックオブジェクトメソッドも同じ仕組みで対応予定だが、`Range` の2引数版自体は別途新機能として実装が必要。
