...
- ❌ **未実装: 可変長 `String` を含む UDT**: VBAのPut/Get仕様ではUDT要素として長さディスクリプターを伴って処理される。現行エンジンはこのディスクリプターの直列化・復元を未実装で、Error 13 として拒否している。多次元配列のバイト順とCP932固定長文字列の物理レイアウトは実Excelで照合済み（`EVAL_LOG.md` XL-001〜XL-007）。
