...
- ✅ **仕様バグ修正（2026-06-25）: `eval('x + 1')`（単独）と `eval('... : x + 1')`（複数文の最後）の結果が一致しない**
  - 前提（実 VBA 仕様の確認）: VBA の statement 文法では、識別子で始まり `+`/`-`（および `=`）が続く裸の文は、単独行でも複数文中でも**常に同一の解釈**になる。一方 `<`/`&`/`And` 等の比較・連結・論理演算子は statement としては成立せず Parse error になるため、この曖昧性自体が存在しない（`x < 10` を文として解析するとパースエラー）。検証結果:
    ...
    | `Foo(1) + 1` / `x.Bar + 1` | `CallStatement`（明示括弧・メンバアクセスが左辺でも同様） |
  ...
  - 修正: 高速パスの判定に、トップレベルが `+`/`-` の `BinaryExpression` で、左辺を `+`/`-` の連鎖を通して再帰的に辿った最終的な葉が `Identifier`/`CallExpression`/`MemberExpression`（= 呼び出し可能な形）である場合も「statement として解析すべき」に含めるよう拡張。括弧で明示的に囲まれた場合（`(x) + 1`）は曖昧性が解消されるため対象外とする | `eval-statement-ambiguity.test.ts`
...
### ✅ B-9: クラスの Private/Public フィールドが UDT 型の場合、既定値が Empty のままで Error 91（2026-06-26 修正済み）

- **症状**: `Private mStats As StatBlock`（UDT 型）のようなクラスフィールドが既定値 `Empty` のまま初期化され、`Class_Initialize` 内で `mStats.Strength = 5` のようなメンバー代入をすると `Error 91: Object variable or With block variable not set` になる。`Dim` 変数の UDT 初期化（`instantiateType()` 経由）は B-4 などで対応済みだったが、クラスフィールドの初期化（`createInstanceFromDef`）は別の独立したコードパスで、文字列・数値系の数種類の型しか default 値を設定していなかった
- **修正**: `createInstanceFromDef` のフィールド既定値判定に `this.env.getType(decl.objectType)` チェックを追加し、UDT 型なら `instantiateType()` で正しく初期化するよう変更（B-4 と同じヘルパーを再利用）。ネストした UDT・複数インスタンス間の独立性（参照共有なし）も確認済み
- | `udt-type.test.ts` (Test 10〜12)
