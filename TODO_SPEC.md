...
- ⚠️ **組み込み関数に VBA パラメーターメタデータ（`BuiltinParamSpec[]`）を持たせる仕組みを導入（実装中、段階移行）**
  ...
  - 残作業: 約140個ある残りの組み込み関数登録箇所の段階移行（Batch 3: `Mid`/`Left`/`Right`/`Format`/`MsgBox`/`InStrRev`/`Replace`/`StrComp`等の複数引数・Optional引数を持つ関数 → Batch 4: ファイル操作・財務・レジストリ・日時の残り）。`CallByName`（ParamArray）は対象外検討中。`MockWorksheet.Range`（1引数/2引数で意味が変わる、かつ2引数版は未実装）等のモックオブジェクトメソッドも同じ仕組みで対応予定だが、`Range` の2引数版自体は別途新機能として実装が必要。
