...
| 状態 | 優先度 | 項目 | 内容 |
|------|--------|------|------|
| ❌ | P2 | **Mid 代入文** (§5.4.3.8) | `Mid(str, 1, 3) = "XYZ"` の代入文形式。`KeywordMid` はレクサーに存在するがパーサーに `parseMidStatement` がなく、関数呼び出し形式のみ対応。 |
| ❌ | P3 | **Def-Directive** (§5.2.2) | `DefInt A-Z` / `DefStr B` 等の暗黙型宣言。レクサー・パーサーともに未対応。古い VBA コードで使用される。 |
| ❌ | P3 | **AddressOf のメンバー形式** (§5.6.16.8) | `AddressOf Module.Proc` の形式が未対応。単純名 `AddressOf Proc` のみ実装。BNF では `member-access-expression` も許容。 |
| ⚠️ | P3 | **Type/Enum ボディ内の Rem コメント** (§5.2.3.3/5.2.3.4) | BNF では `udt-element = rem-statement / udt-member`。現実装は Rem トークンをメンバー名として読もうとしてパースエラーになる。 |
| ⚠️ | P3 | **Optional パラメータの順序チェックなし** (§5.3.1.5) | `Sub F(x, Optional y, z)` のように Optional の後に非 Optional が続いても拒否しない。VBA では実行前コンパイルエラーになるべき。 |
...
### ⚠️ B-3: `Class_Terminate` の早期呼び出し（暫定修正 / 参照カウント未実装）

- **暫定修正**: `triggerTerminate` の呼び出し条件を `Set x = Nothing`（明示的な Nothing 代入）のときのみに限定。オブジェクト間の代入時（`Set x = anotherObj`）では呼ばない。
- **既知の制約 (TODO)**: VBA の正しい仕様は参照カウント方式。ローカル変数がプロシージャスコープを抜けた際の自動 Terminate は未実装。参照カウントを実装するには、すべてのオブジェクト代入でカウントを管理する必要がある（大規模な改修）。
- | `circular-reference-terminate.test.ts` (Test 11: B-3 — Dictionary 格納済みオブジェクトの早期 Terminate が発生しない)
