...
## サンプルコード検証で発見したエンジンバグ（未修正）

`sample/workspace/` の Shop サンプル（Product.cls / Inventory.cls / ShopForm.cls / ShopMain.bas）を実行した際に発見。

### B-1: `parseAsClass` 時にクラス内 `Private Const` が実行時に 0 として解決される

- **症状**: `Private Const MAX_PRICE As Long = 1000000` を宣言しても、クラスメソッド内で参照すると値が `0` になる
- **再現**: `parseAsClass` オプションで `.cls` ファイルをパースし、クラスメソッド内で自クラスの Private Const を参照
- **影響**: `Price Let` 内の `If v > MAX_PRICE Then` が常に True になり不正エラーが発生

### B-2: `parseAsClass` 時にクラス内プライベートメソッドを同クラスメソッドから呼べない

- **症状**: `Private Function ValidateRange(...)` を同クラスの `Price Let` から呼ぶと Error 35 "Sub or Function not defined"
- **再現**: `parseAsClass` でパースしたクラスのメソッドが `Private` ヘルパーを呼び出すケース
- **影響**: クラス内でプライベートメソッドを使った分割設計ができない

### B-3: `Scripting.Dictionary` に格納したクラスインスタンスの参照カウントが誤りで `Class_Terminate` が早期呼び出しされる

- **症状**: ループ内でローカル変数 `p` を Product インスタンスに代入 → Dictionary に格納 → 次ループで `p` に別インスタンスを代入した際、Dictionary が参照を保持しているにも関わらず `Class_Terminate` が呼ばれる
- **再現**: `For i = 0 To N: Set p = New Product: dict.Add key, p: Next i`
- **影響**: Class_Terminate の出力が想定外タイミングで現れる。さらに Terminate 後にオブジェクトへのアクセスが Error 91 を引き起こす可能性

### B-4: UDT 固定長配列要素へのフィールド代入が Error 91

- **症状**: `Dim specs(4) As ProductSpec: specs(0).PName = "緑茶"` が Error 91 "Object variable not set"
- **再現**: `Type` で定義した UDT を固定長配列として宣言し、インデックスでフィールドに代入
- **影響**: UDT 配列パターン全般が使えない
