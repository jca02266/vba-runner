...
- ✅ **Fix: `Dim a(n)` 配列サイズに変数を指定してもエラーにならないバグ** | `dim-const.test.ts`
  ...
  - モジュールレベル: `evaluateVariableDeclaration()`（Pass 2 prerun）で検出。プロシージャ内: `checkConstantArrayBoundsInProc()`（`precheckProc`）で AST 走査により検出
...
- ❌ **`precheckProc` のコンパイルエラーメッセージ "Compile error:" プレフィックスをキャッチ側で一元付与**
  - 現状: 各チェックメソッドがそれぞれ `throwCompileError(code, 'message', line)` を呼んでおり、"Compile error:" プレフィックスは `throwCompileError` 内部で付与されている
  - 改善: 各チェックは素のエラー（プレフィックスなし）をスローし、`precheckProc` の呼び出し元（または `precheckProc` 自身）のキャッチブロックで "Compile error:" プレフィックスと行番号を一元付与する設計にする
