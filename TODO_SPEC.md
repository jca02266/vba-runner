...
| ⚠️ | §5.6.5 | `1.5D10`（VBA Double `D` 指数表記）→ レキサーが `D`/`d` を未対応（§3.x レキサー問題）。**実 VBA 実機確認済み（2026-07-19）**: `x = 1.5D+10` / `y = 2D5` とも正常にコンパイル・実行され `TypeName` は `Double`（`E` 指数表記と同じ扱い）。パースエラーで対象モジュール全体のロードが失敗するため実害は大きい。レキサーの指数部トークン化に `D`/`d` を `E`/`e` と同様に追加すれば直りそうで、実装コストは低い | 科学技術計算 |
...
- ❌ **未実装: `Byte` 配列への文字列の暗黙代入**（`Dim b() As Byte : b = "AB"`）
  ...
  - vba-runner の現状: 代入文自体はエラーにならず通るが、配列の中身が空のまま（`UBound`/要素アクセスで **Error 9** Subscript out of range）。EVAL_LOG.md Bug #25-4 として既知
  - 実装するなら、文字列代入時に対象変数が `Byte` 配列だと判定した場合、UTF-16LE エンコードでバイト配列に変換する処理を代入パスに追加する
